...
export function safeApply(fn, text, onFailure) {
  if (typeof fn !== "function") return text;
  try {
    const out = fn(text);
    if (typeof out !== "string") { onFailure?.(); return text; }
    return out;
  } catch (err) {
    // Rust: eprintln!("[rtk] warning: filter panicked — passing through raw output")
    const name = fn.filterName || fn.name || "anonymous";
    onFailure?.();
    console.warn(`[rtk] warning: filter '${name}' panicked — passing through raw output`);
    return text;
  }
}

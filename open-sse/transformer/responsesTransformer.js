  ...
  const closeMessage = (controller, idx) => {
    if (state.msgItemAdded[idx] && !state.msgItemDone[idx]) {
      ...
      emit(controller, "response.output_text.done", {
        ...
        item_id: msgId,
        output_index: state.msgOutIdx[idx] ?? parseInt(idx),
        content_index: 0,
        text: fullText,
        logprobs: []
      });
  ...
  const closeToolCall = (controller, idx) => {
    ...
    if (callId && !state.funcItemDone[idx]) {
      ...
      emit(controller, "response.function_call_arguments.done", {
        ...
        item_id: `fc_${callId}`,
        output_index: state.funcOutIdx[idx] ?? parseInt(idx),
        arguments: args
      });
        ...
        if (delta.content) {
          ...
          if (content) {
            if (!state.msgItemAdded[idx]) {
              ...
              state.msgOutIdx[idx] = state.nextOutputIndex++;
              const msgId = `msg_${state.responseId}_${idx}`;
              
              emit(controller, "response.output_item.added", {
                type: "response.output_item.added",
                output_index: state.msgOutIdx[idx],
        ...
        if (delta.tool_calls) {
          ...
          for (const tc of delta.tool_calls) {
            ...
            if (!state.funcCallIds[tcIdx] && newCallId) {
              ...
              state.funcOutIdx[tcIdx] = state.nextOutputIndex++;
              
              emit(controller, "response.output_item.added", {
                type: "response.output_item.added",
                output_index: state.funcOutIdx[tcIdx] ?? tcIdx,
                item: {
                  id: `fc_${newCallId}`,
                  type: "function_call",
                  arguments: "",

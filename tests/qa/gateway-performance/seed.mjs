...
const settings = { requireApiKey: true, requireLogin: true, rtkEnabled: false, headroomEnabled: false, pxpipeEnabled: false, contextStructureEnabled: process.env.BENCH_STRUCTURE_ENABLED !== '0', storeRequestDetails: false, backgroundTokenRefreshEnabled: false };
...
const stats = { driver: db.driver, sqlite: db.get('SELECT sqlite_version() AS version').version, journal: db.get('PRAGMA journal_mode'), connections: db.get('SELECT count(*) AS n FROM providerConnections').n, settings };

...
const binary=nativeBinary(),sandbox=nativeSandbox(root);
const args=[...sandbox.prefix,binary,'--init-only','--settings',join(root,'settings.json'),'--setting-sources','','--strict-mcp-config','--mcp-config','{"mcpServers":{}}'];
const env={PATH:process.env.PATH,TMPDIR:process.env.TMPDIR,HOME:join(root,'home'),CLAUDE_CONFIG_DIR:join(root,'config'),DISABLE_AUTOUPDATER:'1',DISABLE_TELEMETRY:'1',DISABLE_ERROR_REPORTING:'1',CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC:'1'};
let output;try{output=execFileSync(sandbox.command,args,{cwd:join(root,'work'),env,encoding:'utf8',timeout:30000,maxBuffer:1024*1024});}catch(error){writeFileSync(join(artifacts,'native-init-error.txt'),String(error.stdout)+'\n'+String(error.stderr));throw error;}
...
writeFileSync(join(artifacts,'native-init.json'),JSON.stringify({binary,args,root,hook,exitCode:0,output,networkPolicy:sandbox.description,platform:process.platform,scope:'native SessionStart invocation only; no native compaction or conversation',providerRequests:0},null,2));

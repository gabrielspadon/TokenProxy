...
For visual iterations use `node scripts/redesign-preview.mjs start --mode dev --scenario representative`. The custom webpack dev server reads current source and writes its complete Next cache inside the owned temporary root. A root-local symlink resolves the existing installed dependencies; shared `.next` is untouched. Startup then probes one deep dynamic route until it resolves, because Next's dev bundler resolves its startup promise on the first watcher aggregation and that aggregation is routinely partial; each retry rewrites the mtime of `src/app/api/admin/compatibility/runs/[id]/cancel/route.js` in the shared tree to force a fresh one. File contents never change, so source-manifest provenance holds, but every other watcher on this checkout, whether a second preview or a plain `npm run dev`, recompiles on each forced re-aggregation. Measured on this host, three concurrent two-preview starts still reached the canary 405 in one or two attempts, because each preview compiles into its own `NEXT_DIST_DIR` and the probe's retry budget absorbs the extra aggregation; prefer one at a time anyway, since the duplicated compile is pure waste. The probe proves that one route resolved, never that the whole route table aggregated. HMR uses only the selected loopback origin. Next's optional npm update check is suppressed in this tooling with installed-version metadata and freshness explicitly unknown. Private environment files remain invisible to the watcher. No production application code changes its behavior to enable this mode.
...
`representative` includes all edge fixtures plus 12 accounts across six provider identities. Eight are enabled as a local policy, with no usable credentials and no fabricated qualification or entitlement. It adds 48 exactly linked request/context/usage/cost histories and synthetic quota observations, six per account beyond the first six, retaining fresh, stale, unknown and exhausted states independently. The seed receipt reports the count as `representative.additionalQuotaObservations`, and `fixtures.test.js` checks it against the `synthetic-fixture` rows in `quotaObservations`; the capacity fixtures seed further rows under `provider-usage`. `verify-preview.mjs ROOT` authenticates normally and verifies real provider, health, quota, model, context and analytics handlers; its receipt is separate from rendered acceptance.

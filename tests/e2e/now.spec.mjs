...
test("a null measure renders as not reported with its reason, never zero", async ({ page }) => {
  await page.goto("/dashboard");
  const failovers = page.locator(".measure", { hasText: "Failovers" });
  await expect(failovers.locator(".unreported")).toHaveText("Not reported");
  await expect(failovers.locator(".why")).not.toHaveText("");
  await expect(failovers).not.toContainText(/\b0\b/);
});

test("admin refusal shapes are rendered as their own sentences", async ({ page }) => {
  await page.route("**/api/admin/quota", (r) => r.fulfill(json(403, { error: "An operator credential is required. An inference API key does not satisfy this endpoint.", code: "forbidden_class", source: "tokenproxy-admin" })));
  await page.route("**/api/admin/health/detail", (r) => r.fulfill(json(500, { error: "db locked", code: "state_unavailable", source: "tokenproxy-admin" })));
  await page.goto("/dashboard");
...
test("no session identity is rendered from the stream", async ({ page }) => {
  await page.route("**/api/usage/stream*", (r) => r.fulfill({

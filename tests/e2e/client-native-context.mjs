...
const policy='(version 1) (allow default) (deny network*) (deny file-read* (subpath "/Users/gabrielspadon/.claude") (subpath "/Users/gabrielspadon/.ssh")) (deny process-exec (literal "/usr/bin/security"))';
writeFileSync(join(root,'no-network.sb'),policy);writeFileSync(join(root,'settings.json'),JSON.stringify({autoCompactWindow:1000000,env:{CLAUDE_AUTOCOMPACT_PCT_OVERRIDE:'100'}}));
const cases=[];
for(const model of ['claude-fable-5','claude-fable-5[1m]','claude-fable-5-1','claude-fable-5-1[1m]','claude-opus-5','claude-opus-5[1m]']){
 const debug=join(root,`${cases.length}.debug`),args=['-f',join(root,'no-network.sb'),binary,'--bare','--print','/context','--model',model,'--settings',join(root,'settings.json'),'--debug-file',debug,'--setting-sources','','--strict-mcp-config','--mcp-config','{"mcpServers":{}}'];
 const output=execFileSync('/usr/bin/sandbox-exec',args,{cwd:join(root,'work'),env:{PATH:process.env.PATH,TMPDIR:process.env.TMPDIR,CLAUDE_CONFIG_DIR:join(root,'config'),ANTHROPIC_BASE_URL:'http://127.0.0.1:9',ANTHROPIC_AUTH_TOKEN:'synthetic-no-provider',DISABLE_AUTOUPDATER:'1',CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC:'1'},encoding:'utf8',timeout:20000,maxBuffer:1048576});
...
writeFileSync(join(artifacts,'native-context.json'),JSON.stringify({binary,root,scope:'native built-in /context in isolated gateway configuration; no conversation',settings:{autoCompactWindow:1000000,percentOverride:100},networkPolicy:policy,providerRequests:0,cases},null,2));

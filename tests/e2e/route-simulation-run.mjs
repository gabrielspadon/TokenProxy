import { prepareRouteSimulationFixture } from '../../scripts/qa/browser-workflow-fixtures.mjs';
import { execFileSync } from 'node:child_process';
import { writeFileSync, mkdirSync } from 'node:fs';
import { resolve, join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
const project = resolve(dirname(fileURLToPath(import.meta.url)), '../..');
const artifacts = resolve(process.argv[2] || '/tmp/route-simulation-evidence');
...
prepareRouteSimulationFixture(seeded, artifacts);
let started = false;
try {
  ...
  execFileSync(
    process.execPath,
    [join(project, 'tests/e2e/route-simulation-acceptance.mjs'), seeded.root, artifacts],

...
describe('DB Concurrency — atomic safety', () => {
  ...
  it('200 parallel saveRequestDetail → all flushed', async () => {
    ...
    // The write buffer flushes immediately once it crosses batchSize (10), so
    // every push above N=200 already triggered a flush; poll briefly for the
    // trailing under-threshold remainder instead of sleeping a fixed 6s for a
    // flush that is normally done in well under 100ms.
    let list;
    for (let waited = 0; waited < 3000; waited += 25) {
      list = await db.getRequestDetails({ provider: 'openai', pageSize: 500 });
      if (list.pagination.totalItems >= N) break;

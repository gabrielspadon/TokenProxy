...
it('retains disabled byte stages and final size without serializing those stages', async () => {
  ...
  expect(disabled.map(stage => stage.stage)).toEqual(['schema', 'thinking', 'rtk', 'privacy', 'inject', 'pxpipe', 'mem', 'headroom', 'qac', 'pairs', 'diet', 'epochMicro', 'epochAuto', 'reorder', 'midinject']);

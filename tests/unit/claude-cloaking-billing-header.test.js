...
// The version in the header is whatever the spoofed UA currently carries; pin
// SHAPE and agreement with that source of truth, never a version literal
// (claude-cli-ua-version-floor.test.js owns the numeric floor).
const SPOOFED_VERSION = /claude-cli\/(\d+\.\d+\.\d+)/.exec(
...
describe('applyCloaking billing header determinism', () => {
  it('produces a byte-identical header across calls with the same key+session and different bodies', () => {
    ...
    const b = applyCloaking(
      bodyWith('a much longer second message with different content'),
      OAT_KEY,
      'session-1'
    );
    ...
    expect(a.system[0].text).toMatch(
      new RegExp(
        `^x-anthropic-billing-header: cc_version=${escapeRe(SPOOFED_VERSION)}\\.[0-9a-f]{3}; cc_entrypoint=sdk-cli; cch=[0-9a-f]{5};$`
      )
    );
  });

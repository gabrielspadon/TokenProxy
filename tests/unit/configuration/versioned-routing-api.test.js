...
const fixture = vi.hoisted(() => ({ db: null, operator: false, inference: false, loopback: true, reads: 0 }));
vi.mock('../../../src/lib/db/driver.js', () => ({ getAdapter: async () => { fixture.reads++; return fixture.db; } }));
vi.mock('@/dashboardGuard', () => ({ hasValidCliToken: async () => fixture.operator, isLocalRequest: () => fixture.loopback }));
vi.mock('@/lib/auth/dashboardSession', () => ({ verifyDashboardAuthToken: async () => false }));
vi.mock('@/lib/auth/clientApiKey', () => ({ resolveClientApiKey: async () => ({ valid: fixture.inference }) }));
...
afterEach(() => { vi.restoreAllMocks(); fixture.db.close(); });
async function call(method, path = '', body, raw) {
  const request = new NextRequest(`http://127.0.0.1/api/admin/configuration${path ? '/' + path : ''}`, { method, ...(method === 'GET' ? {} : { body: raw ?? JSON.stringify(body), headers: { 'content-type': 'application/json' } }) });
  const response = await ({ GET, POST, PATCH }[method])(request, { params: Promise.resolve({ path: path ? path.split('?')[0].split('/') : [] }) });
  return { status: response.status, headers: response.headers, body: await response.json() };
}
const document = { combos: [{ id: 'plan-1', name: 'work', kind: null, models: ['openai/gpt-4o'] }], aliases: {}, settings: { comboStrategy: 'fallback' } };
...
])('refuses auth class %s/%s loopback %s for %s %s before config access', async (operator, inference, loopback, method, path, status) => {
  Object.assign(fixture, { operator, inference, loopback });
  const before = fixture.db.raw.serialize();
  const result = await call(method, path, { document });
  expect(result.status).toBe(status);
  expect(result.headers.get('cache-control')).toBe('no-store');
  expect(fixture.reads).toBe(0);
  expect(fixture.db.raw.serialize()).toEqual(before);
});
it('executes actual authenticated create/revise/validate/activate/conflict/rollback/read/audit', async () => {
  ...
  const created = await call('POST', 'drafts', { document, expectedCurrent: current.body.currentHash });
  expect(created.status).toBe(201);
  expect(created.body.receipt.provenance).toEqual({ source: 'admin.configuration', actorClass: 'operator', actorId: null });
  const id = created.body.id;
  const revisedDoc = structuredClone(document); revisedDoc.settings.comboStrategy = 'round-robin';
  expect((await call('PATCH', `drafts/${id}`, { document: revisedDoc, expectedRevision: 1 })).status).toBe(200);
  expect((await call('POST', `drafts/${id}/validate`, { expectedRevision: 2 })).body.valid).toBe(true);
  const activated = await call('POST', `drafts/${id}/activate`, { expectedRevision: 2, expectedCurrent: current.body.currentHash });
  expect(activated.status).toBe(200);
  expect((await call('GET')).body.document.settings.comboStrategy).toBe('round-robin');
  expect((await call('POST', `drafts/${id}/activate`, { expectedRevision: 2, expectedCurrent: current.body.currentHash })).status).toBe(409);
  const beforeVersion = created.body.version.parentVersionId;
  const rollback = await call('POST', `versions/${beforeVersion}/rollback`, { expectedCurrent: activated.body.currentHash });
...
it('does not expose raw database failure messages', async () => {
  vi.spyOn(fixture.db, 'all').mockImplementation(() => { throw new Error('Bearer credentials-at-host.invalid'); });

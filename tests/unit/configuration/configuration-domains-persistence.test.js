...
import { configurationDomains } from '../../../src/lib/db/repos/configurationDomainsRepo.js';
const { getCurrentConfiguration, createConfigurationDraft, activateConfigurationDraft } =
...
async function draft() {
  ...
  return {
    current,
    draft: await createConfigurationDraft({ document, expectedCurrent: current.currentHash }),
  };
}
it('strict sql.js flush is immediately reloadable with private permissions and no pending timer', async () => {
  ...
  const result = await activateConfigurationDraft(d.id, {
    expectedRevision: 1,
  ...
  try {
    expect(
      JSON.parse(reopened.get('SELECT data FROM settings WHERE id=1').data).connectTimeoutMs
    ).toBe(7000);
    const latest = reopened.get(
      'SELECT outcome, details FROM configReceipts ORDER BY id DESC LIMIT 1'
    );
...
it('flush failure before staging persistence leaves routing unchanged', async () => {
  ...
  const rename = vi.spyOn(fs, 'renameSync').mockImplementation(() => {
    throw new Error('ENOSPC-secret');
  });
  await expect(
    activateConfigurationDraft(d.id, { expectedRevision: 1, expectedCurrent: current.currentHash })
  ).rejects.toMatchObject({
    code: 'configuration_stage_persistence_failed',
...
it('post-commit flush failure reports partial and preserves recoverable memory plus durable staged receipt', async () => {
  ...
  expect(result).toMatchObject({
    outcome: 'partial',
    completion: { databaseCommitted: true, persistence: 'failed' },
  });
  expect(
    JSON.parse(fixture.db.get('SELECT data FROM settings WHERE id=1').data).connectTimeoutMs
  ).toBe(7000);
  const reopened = await createSqlJsAdapter(file);
  try {
    expect(reopened.get('SELECT data FROM settings WHERE id=1')).toBeUndefined();
    expect(
      reopened.get('SELECT outcome FROM configReceipts ORDER BY id DESC LIMIT 1').outcome
    ).toBe('staged');
  } finally {
    reopened.close();
  }
  ...
  try {
    expect(
      JSON.parse(recovered.get('SELECT data FROM settings WHERE id=1').data).connectTimeoutMs
    ).toBe(7000);
  } finally {
    recovered.close();
  }
  ...
  try {
    ...
    vi.spyOn(fixture.db, 'run').mockImplementation((sql, args) => {
      ...
      if (sql.startsWith('INSERT INTO settings(')) {
        observed = true;
        expect(other.get('SELECT data FROM settings WHERE id=1')).toBeUndefined();
      }
      return result;
    });
    const applied = await activateConfigurationDraft(d.id, {
      expectedRevision: 1,
      expectedCurrent: current.currentHash,
    });
    expect(observed).toBe(true);
    expect(
      JSON.parse(other.get('SELECT data FROM settings WHERE id=1').data).connectTimeoutMs
    ).toBe(7000);
    vi.restoreAllMocks();
    const state = await getCurrentConfiguration();
    const next = await createConfigurationDraft({
      document: {
        ...state.document,
        network: { ...state.document.network, settings: { outboundProxyEnabled: true } },
      },
      expectedCurrent: applied.currentHash,
    });
    ...
    vi.spyOn(fixture.db, 'transaction').mockImplementation((fn) => {
      ...
      if (++transactions === 1)
        other.run('UPDATE settings SET data = ? WHERE id = 1', ['{"connectTimeoutMs":9000}']);
      return result;
    });
    await expect(
      activateConfigurationDraft(next.id, {
        expectedRevision: 1,
        expectedCurrent: applied.currentHash,
      })

...
vi.mock('@/shared/workspace/useResource', () => ({
  useResource: (url) => {
    ...
    return url
      ? state.own
      : { data: null, loading: false, error: null, refresh: () => {}, receivedAt: null };
  },
}));
vi.mock('@/shared/workspace/AnalyticalChart', async () => ({
  ...
  AnalyticalChart: (props) => {
    state.chart = props;
    return <div aria-label={props.label} />;
  },
}));
import {
  CALENDAR_METRICS,
  CapacityActivity,
  capacityActivityOption,
  capacityBucketScope,
  capacityCalendarDays,
  capacityCalendarLevel,
  capacityCalendarMonth,
  capacityCalendarRange,
  capacityCalendarReading,
  capacityDayScope,
  capacityMonthStart,
  capacityRefreshStatus,
  capacityShiftMonth,
  capacityTokensOption,
} from '@/app/dashboard/CapacityActivity';
let root, host;
...
beforeEach(() => {
  ...
  const points = Array.from({ length: 8 }, (_, index) => ({
    bucketStartMs: start + index * 60000,
    bucketStart: new Date(start + index * 60000).toISOString(),
    logicalRequests: 2,
    records: 3,
    failed: 1,
    cacheReadTokens: 900,
  ...
  state.workspace = {
    scope: {
      provider: 'codex',
      start: new Date(start + 30000).toISOString(),
      end: new Date(start + 8 * 60000).toISOString(),
    },
    setScope: vi.fn(),
    activity: {
      url: '/api/activity?scope=fixture',
      refresh: vi.fn(),
      loading: false,
      receivedAt: '2026-09-07T12:09:00Z',
      data: {
        source: 'requestStats',
        summary: {
          records: 24,
...
it('keeps logical request totals, attempts and paired cache token share distinct', async () => {
  ...
  expect(summary).toEqual([
    'Requests with an ID10',
    'Attempts24',
    'Failed attempts8',
    'Cache read share of input75%',
  ]);
  ...
  expect(
    state.chart.option.series.every((series) => series.showSymbol && series.showAllSymbol)
  ).toBe(true);
  expect(state.chart.option.tooltip).toMatchObject({
    renderMode: 'html',
    appendTo: 'body',
    confine: false,
  });
  await act(async () => host.querySelector('[value="tokens"]').click());
  expect(state.chart.option.series.map((series) => series.name)).toEqual([
    'Tokens in',
    'Tokens out',
    'Cache read',
    'Cache write',
  ...
  expect([...host.querySelectorAll('[role="listitem"]')].map((node) => node.textContent)).toEqual([
  ...
  expect([...host.querySelectorAll('tbody th')].map((node) => node.getAttribute('abbr'))).toEqual([
    'Monday',
    'Tuesday',
    'Wednesday',
    'Thursday',
  ...
  expect(
    [...host.querySelectorAll('[data-day]')].filter((node) => node.tabIndex === 0)
  ).toHaveLength(1);
});
...
it('offers a scale and a style for the time charts, and four fixed metrics for the calendar', async () => {
  ...
  expect(
    state.chart.option.series.every((series) => series.type === 'line' && series.areaStyle)
  ).toBe(true);
  await act(async () => root.unmount());
  ...
  state.own = {
    ...state.own,
    data: { ...state.own.data, series: { ...state.own.data.series, bucketMs: 3600000 } },
  };
  root = createRoot(host);
  await render();
  ...
  expect(host.textContent).toContain(
    '1-hour intervals (the 1-minute scale is too fine for this period)'
  );
  expect(
    CALENDAR_METRICS.map((item) => [item.label, item.value, item.samples, item.token])
  ).toEqual([
...
it('sums every metric per UTC day and keeps an unreported metric out of the totals', () => {
  const points = [
    {
      bucketStartMs: Date.parse('2026-09-07T23:30:00Z'),
      cacheReadTokens: 10,
      cacheReadSamples: 1,
      cacheWriteTokens: 4,
      cacheWriteSamples: 1,
    ...
    {
      bucketStartMs: Date.parse('2026-09-07T23:45:00Z'),
      cacheReadTokens: 5,
      cacheReadSamples: 1,
      cacheWriteTokens: 9,
      cacheWriteSamples: 0,
  ...
  expect(capacityDayScope('2026-09-08')).toEqual({
    period: 'custom',
    start: '2026-09-08T00:00:00.000Z',
    end: '2026-09-09T00:00:00.000Z',
  });
  expect(capacityDayScope('nope')).toBeNull();
  const tokens = capacityTokensOption(
    [{ bucketStartMs: start, inputTokens: 40, inputSamples: 2, outputTokens: 0, outputSamples: 0 }],
    60000
  );
...
it('separates a reported zero, an unreported metric and a day with no attempts at all', () => {
  const points = [
    ...
    {
      bucketStartMs: Date.parse('2026-09-02T04:00:00Z'),
      inputTokens: 0,
      inputSamples: 3,
      outputTokens: 0,
      outputSamples: 3,
    ...
    {
      bucketStartMs: Date.parse('2026-09-03T04:00:00Z'),
      inputTokens: 1200,
      inputSamples: 2,
      outputTokens: 300,
      outputSamples: 2,
  ...
  expect(zero.values.map((item) => [item.label, item.reported, item.text])).toEqual([
    ['Tokens in', true, '0 tokens'],
    ['Tokens out', true, '0 tokens'],
    ['Cache read', true, '0 tokens'],
    ['Cache write', true, '0 tokens'],
  ]);
  expect(zero.label).toBe(
    'Wednesday, 2 September 2026. Tokens in 0 tokens. Tokens out 0 tokens. Cache read 0 tokens. Cache write 0 tokens.'
  );
  ...
  expect(
    silent.values.every((item) => item.reported === false && item.text === 'not reported')
  ).toBe(true);
  expect(silent.label).toBe(
    'Friday, 4 September 2026. No recorded attempts. Tokens in not reported. Tokens out not reported. Cache read not reported. Cache write not reported.'
  );
  ...
  expect(
    [1, 250, 251, 500, 501, 750, 751, 1000].map((value) => capacityCalendarLevel(value, 1000))
...
it('aligns weekdays and dates across month, year and timezone boundaries', () => {
  ...
  expect(capacityShiftMonth(Date.parse('2026-03-31T12:00:00Z'), 1)).toBe(
  ...
  expect(march.days.map((cell) => cell.day).slice(0, 3)).toEqual([
    '2026-03-01',
    '2026-03-02',
    '2026-03-03',
  ]);
  ...
  const boundary = capacityCalendarMonth(
    [{ bucketStartMs: Date.parse('2026-03-31T23:59:00Z'), inputTokens: 5, inputSamples: 1 }],
...
it('scales each metric against its own month maximum so a small metric stays visible', () => {
  const points = [
    {
      bucketStartMs: Date.parse('2026-09-01T04:00:00Z'),
      outputTokens: 40,
      outputSamples: 1,
...
it('reports a partially covered metric as a total plus its interval coverage', () => {
  ...
  expect(capacityCalendarReading(month.days[4]).values[0].text).toBe(
    '90 tokens (1 of 2 intervals reported)'
  );
});
...
it('says when this panel is not refreshing itself rather than letting a quiet page imply quiet traffic', () => {
  ...
  expect(
    capacityRefreshStatus({ mode: 'live', background: true, historical: false }, null)
  ).toBeNull();
  expect(capacityRefreshStatus({ mode: 'summary', background: false }, null)).toContain(
    'does not refresh on its own'
  );
  expect(
    capacityRefreshStatus(
      { mode: 'paused', background: false, pausedAt: '2026-09-07T12:00:00Z' },
      null
    )
  ).toBe(
    'Updates paused at 07 Sept, 12:00 UTC. This panel is not refreshing, so later activity is not shown here.'
  );
  expect(
    capacityRefreshStatus({ mode: 'live', background: false, historical: true }, null)
  ).toContain('does not refresh');
  expect(
    capacityRefreshStatus(
      { mode: 'live', background: false },
      { capturedAt: '2026-09-01T00:00:00Z' }
    )
  ).toContain('Fixed snapshot');
});

it('preserves missing token samples and sparse gaps instead of drawing measured zeros', () => {
  const option = capacityActivityOption(
    [
      { bucketStartMs: start, cacheReadTokens: 0, cacheReadSamples: 0 },
      { bucketStartMs: start + 120000, cacheReadTokens: 60, cacheReadSamples: 1 },
    ],
    60000
  );
...
it('bounds both tracks to the full covered minute when only one bucket remains', () => {
  ...
  const option = capacityActivityOption(
    [{ bucketStartMs: singleStart, logicalRequests: 1, records: 1 }],
    60000
  );
...
it('keeps multi-day ticks distinct and supports seconds only for sub-minute buckets', () => {
  const days = capacityActivityOption(
    [{ bucketStartMs: start }, { bucketStartMs: start + 2 * 86400000 }],
    60000
  ).xAxis[1];
...
it('includes the final bucket end across multiple sparse buckets without changing observations', () => {
  const points = [
    { bucketStartMs: start, records: 2 },
    { bucketStartMs: start + 120000, records: 1 },
  ];
...
it.each([0, -60000, null, undefined, NaN, Infinity])(
  'leaves the time domain unset for invalid bucket width %s',
  (bucketMs) => {
    const points = [
      { bucketStartMs: start, records: 1 },
      { bucketStartMs: start + 120000, records: 2 },
    ];
...
it('leaves empty or overflowing time domains unset', () => {
  for (const option of [
    capacityActivityOption([], 60000),
    capacityActivityOption([{ bucketStartMs: Number.MAX_VALUE }], Number.MAX_VALUE),
  ]) {
    for (const axis of option.xAxis) {
      expect(axis.min).toBeUndefined();
      expect(axis.max).toBeUndefined();
...
it('selects a clipped interval from a chart click and a whole day from a calendar cell', async () => {
  ...
  expect(state.workspace.setScope).toHaveBeenLastCalledWith({
    period: 'custom',
    start: new Date(start + 30000).toISOString(),
    end: new Date(start + 60000).toISOString(),
  });
  ...
  expect(
    capacityBucketScope({ bucketStartMs: start - 120000 }, 60000, state.workspace.scope)
  ).toBeNull();
...
it('does not turn entirely unattributed history into a known zero requests count', async () => {
  state.workspace.activity.data.summary = {
    records: 2,
    logicalRequests: 0,
    unattributedAttempts: 2,
    failed: 0,
    cacheReadFraction: null,
  };
  await render();
...
const CALENDAR_FIXTURE = [
  {
    bucketStartMs: Date.parse('2026-09-01T06:00:00Z'),
    records: 4,
    inputTokens: 9_876_543_210,
    inputSamples: 4,
  ...
  {
    bucketStartMs: Date.parse('2026-09-09T06:00:00Z'),
    records: 2,
    inputTokens: 0,
    inputSamples: 2,
    outputTokens: 0,
  ...
  {
    bucketStartMs: Date.parse('2026-09-17T06:00:00Z'),
    records: 3,
    inputTokens: 5_000,
    inputSamples: 3,
    outputTokens: 700,
  ...
  {
    bucketStartMs: Date.parse('2026-09-30T23:30:00Z'),
    records: 1,
    inputTokens: 1_500,
    inputSamples: 1,
    outputTokens: 90,
...
const calendarWorkspace = (patch = {}) => {
  ...
  state.own = {
    ...state.own,
    receivedAt: '2026-09-30T23:59:00Z',
    data: { ...state.own.data, series: { bucketMs: 86400000, points: CALENDAR_FIXTURE } },
  };
};
const quadrants = (day) =>
  [...host.querySelector(`[data-day="${day}"]`).querySelectorAll('span > b')].map((node) =>
    node.dataset.absent ? 'absent' : `level-${node.dataset.level}`
  );
...
it('renders four quadrants for every day, hatching only what no bucket reported', async () => {
  ...
  expect(host.querySelector('[data-day="2026-09-09"]').getAttribute('aria-label')).toBe(
    'Wednesday, 9 September 2026. Tokens in 0 tokens. Tokens out 0 tokens. Cache read 0 tokens. Cache write 0 tokens.'
  );
  expect(host.querySelector('[data-day="2026-09-17"]').getAttribute('aria-label')).toBe(
    'Thursday, 17 September 2026. Tokens in 5,000 tokens. Tokens out 700 tokens. Cache read 2,000 tokens. Cache write not reported.'
  );
  expect(host.querySelector('[data-day="2026-09-05"]').getAttribute('aria-label')).toContain(
    'No recorded attempts.'
  );
  // A large count keeps its digits rather than becoming a compact glyph.
  expect(host.querySelector('[data-day="2026-09-01"]').getAttribute('aria-label')).toContain(
    '9,876,543,210 tokens'
  );
...
it('gives all four exact values on hover and on keyboard focus, and moves by arrow key', async () => {
  ...
  await act(async () =>
    host
      .querySelector('[data-day="2026-09-17"]')
  ...
  const press = (key) =>
    act(async () =>
      host
        .querySelector(`[data-day="${document.activeElement.dataset.day}"]`)
        .dispatchEvent(Object.assign(new KeyboardEvent('keydown', { key, bubbles: true }), {}))
    );
  await press('ArrowRight');
  expect(document.activeElement.dataset.day).toBe('2026-09-16');
...
it('shows only the selected period and keeps weekday alignment', async () => {
  calendarWorkspace({
    scope: { period: 'custom', start: '2026-09-09T00:00:00.000Z', end: '2026-09-10T00:00:00.000Z' },
  });
  await render();
  ...
  expect(
    [...host.querySelectorAll('[data-day]')].filter(
      (node) => node.getAttribute('aria-pressed') === 'true'
    )
  ).toHaveLength(1);
  // The selected day's four values are the readout without any pointer.
  expect(host.querySelector('p[aria-live="polite"]').textContent).toContain(
    'Wednesday, 9 September 2026'
  );
  expect(host.querySelectorAll('[data-day]')).toHaveLength(1);
  expect(host.querySelector('[aria-label="Previous month"]')).toBeNull();
  expect(host.querySelectorAll('tbody tr')).toHaveLength(7);
  expect(selected.closest('tr').querySelector('th').getAttribute('abbr')).toBe('Wednesday');
});
...
it('updates under Live and stays put under Summary and Paused', async () => {
  ...
  await act(async () => {
    state.own = {
      ...state.own,
      receivedAt: '2026-09-30T23:59:30Z',
      data: {
        ...state.own.data,
        series: {
          bucketMs: 86400000,
          points: [
            ...CALENDAR_FIXTURE,
            {
              bucketStartMs: Date.parse('2026-09-20T06:00:00Z'),
              records: 1,
              inputTokens: 640,
              inputSamples: 1,
              outputTokens: 12,
  ...
  expect(host.querySelector('[data-day="2026-09-20"]').getAttribute('aria-label')).toContain(
    'Tokens in 640 tokens'
  );
  ...
  for (const [observations, sentence] of [
    ...
    [
      { mode: 'paused', background: false, pausedAt: '2026-09-30T23:50:00Z' },
      'Updates paused at 30 Sept, 23:50 UTC',
    ],
  ]) {
    await act(async () => {
      state.workspace = { ...state.workspace, observations };
      root.render(
        <MantineProvider env="test">
          <CapacityActivity />
        </MantineProvider>
...
it('keeps the requested range honest when no series arrived at all', async () => {
  calendarWorkspace({
    scope: { period: 'custom', start: '2026-09-01T00:00:00Z', end: '2026-10-01T00:00:00Z' },
  });
  state.own = {
    ...state.own,
    receivedAt: '2026-09-30T23:59:00Z',
    data: { ...state.own.data, series: { bucketMs: 86400000, points: [] } },
  };
  await render();
  ...
  expect(
    [...host.querySelectorAll('[data-day] span > b')].every(
      (node) => node.dataset.absent === 'true'
    )
  ).toBe(true);
});
...
it('says a day outside the asked period is unasked rather than unreported', () => {
  ...
  expect(unasked.label).toBe(
    'Tuesday, 1 September 2026. Outside the selected period. Tokens in outside the selected period. Tokens out outside the selected period. Cache read outside the selected period. Cache write outside the selected period.'
  );
  ...
  const away = capacityCalendarMonth([], Date.parse('2026-03-10T00:00:00Z'), {
    ...
    endMs: Date.parse('2026-09-21T00:00:00Z'),
  });
...
it('does not render days outside the query as missing observations', async () => {
  calendarWorkspace({
    scope: { period: 'custom', start: '2026-09-09T00:00:00.000Z', end: '2026-09-10T00:00:00.000Z' },
  });
  await render();
  ...
  expect(host.querySelector('[data-day="2026-09-08"]')).toBeNull();
});

it('bounds long history pages and preserves the same intensity scale across pages', () => {
  const first = Date.parse('2024-01-01T00:00:00Z');
  const last = Date.parse('2026-01-01T00:00:00Z');
  const points = [
    { bucketStartMs: first, inputTokens: 100, inputSamples: 1 },
    { bucketStartMs: last - 86400000, inputTokens: 20, inputSamples: 1 },
  ];
  const early = capacityCalendarRange(points, first, last, 0);
  const recent = capacityCalendarRange(points, first, last);
  expect(early.days).toHaveLength(366);
  expect(recent.days).toHaveLength(365);
  expect(recent.days.at(-1).day).toBe('2025-12-31');
  expect(early.maxima).toEqual(recent.maxima);
  expect(recent.maxima.inputTokens).toBe(100);
  expect(early.totalDays).toBe(731);

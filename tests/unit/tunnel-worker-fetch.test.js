...
// TUNNEL_WORKER_INSECURE bypasses globalThis.fetch and dispatches via a raw
// https.request (workerFetch.js), so mocking fetch alone still reaches the
// real network. node:https is mocked here too, answering every request with
// a canned 200 so the insecure path never leaves the process.
...
describe('tunnel worker fetch (TLS failure)', () => {
  ...
  it('rejects the insecure path for non-worker hosts', async () => {
    process.env.TUNNEL_WORKER_INSECURE = '1';
    const { workerFetch } = await import('../../src/lib/tunnel/cloudflare/workerFetch.js');
    await expect(workerFetch('https://example.com/api/health')).rejects.toThrow(
      /only applies to abc-tunnel\.us/
    );
  });

...
const visibleFixture=createVisibleTelemetryFixture(db,'context-telemetry-pipeline');
const completeFixture=input=>visibleFixture(async()=>{
 const result=await handleChatCore(input);await result.response.text();return result;
});
...
it("persists the actual nonstream provider usage at the shared attempt id",async()=>{
 ...
 const result=await completeFixture(base());expect(result.success).toBe(true);
...
it("records effective epoch and cache controls as explicit booleans",async()=>{
 mocks.execute.mockImplementation(async()=>({response:Response.json({id:"fixture",type:"message",role:"assistant",content:[{type:"text",text:"answer"}],stop_reason:"end_turn",usage:{input_tokens:100,output_tokens:4}})}));
 ...
 const result=await completeFixture(args);expect(result.success).toBe(true);
 const overview=await waitFor(async()=>{const r=await getContextOverview();return r.summary.succeeded?r:null;});
 const {turns}=await getContextSession(overview.sessions[0].id);
...
it("records transport failures without fabricated usage and forbids uncertain replay",async()=>{
 ...
 const result=await completeFixture(base());expect(result.success).toBe(false);expect(result.failureMetadata.safeToReplay).toBe(false);
...
it("records usage-less completed streams as estimates rather than observed counts",async()=>{
 const stream='data: '+JSON.stringify({id:'estimated-fixture',object:'chat.completion.chunk',choices:[{index:0,delta:{content:'estimated answer'},finish_reason:null}]})+'\n\n'
  +'data: '+JSON.stringify({id:'estimated-fixture',object:'chat.completion.chunk',choices:[{index:0,delta:{},finish_reason:'stop'}]})+'\n\ndata: [DONE]\n\n';
 mocks.execute.mockResolvedValue({response:new Response(stream,{headers:{'content-type':'text/event-stream'}})});
 const args=base();args.body.stream=true;args.credentials.sessionHash='c'.repeat(32);
 expect((await completeFixture(args)).success).toBe(true);
 const overview=await waitFor(async()=>{const r=await getContextOverview();return r.summary.succeeded?r:null;});
...
it("scopes cache-prefix measurements by session, provider, model and account",async()=>{
 ...
 for(const account of ["cache-account-a","cache-account-a","cache-account-b"]){const args=base();args.connectionId=account;args.credentials.sessionHash="d".repeat(32);await completeFixture(args);}

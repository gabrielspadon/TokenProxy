...
import EconomicsLens, { EconomicsDetail } from '../../src/shared/components/workspace/EconomicsLens';
import { averageTokens, costShare, formatEstimate, groupFilters, groupKey, groupName, qualityNotes, TOKEN_COLUMNS } from '../../src/shared/components/workspace/economics';

const quantities = {
  records: 10, inputTokens: 1000, uncachedInputTokens: 200, cacheReadTokens: 700,
  cacheWriteTokens: 100, outputTokens: 80, costSamples: 8, zeroCostRows: 3,
  recordedCostUsd: 4, inputSamples: 10, outputSamples: 10, cacheReadSamples: 9,
  cacheWriteSamples: 9, uncachedInputSamples: 9, inconsistentCacheRows: 0,
  missingTokenDetailRows: 0, invalidTokenRows: 0,
};
const providerA = { ...quantities, provider: 'openai', model: 'shared-model', connectionId: 'configured-1' };
const providerB = { ...quantities, provider: 'anthropic', model: 'shared-model', connectionId: 'historical-123456789', recordedCostUsd: 9, records: 20 };
const recordA = {
  id: 19, timestamp: '2026-09-01T12:00:00.000Z', provider: 'openai', model: 'shared-model', connectionId: 'configured-1',
  inputTokens: 1000, uncachedInputTokens: 0, cacheReadTokens: 1700, cacheWriteTokens: 100,
  outputTokens: 80, recordedCostUsd: 0, status: 'success', inconsistentCache: 1,
  missingTokenDetail: 0, invalidTokens: 0,
};
const recordB = { ...recordA, id: 18, timestamp: '2026-09-01T11:00:00.000Z', recordedCostUsd: 0.01 };
const data = {
  summary: { ...quantities, records: 30, recordedCostUsd: 13 }, groups: [providerA, providerB], items: [recordA, recordB],
  pagination: { page: 1, pageSize: 2, totalItems: 30, totalPages: 15, hasNext: true, hasPrev: false },
};

let container;
let root;
function mount(props = {}) {
  act(() => root.render(<MantineProvider env="test"><EconomicsLens data={data} {...props} /></MantineProvider>));
}
...
function byButton(text, parent = container) {
  return [...parent.querySelectorAll('button')].find((element) => element.textContent.includes(text));
}
function tableBody(label) { return container.querySelector(`table[aria-label="${label}"] tbody`); }
...
describe('Economics ledger interactions', () => {
  it('sorts the cohort book numerically and keeps named provider/model identities distinct', () => {
    ...
    click(byButton('Estimate', container.querySelector('table[aria-label="Economics by cohort"]')));
  ...
  it('selects a cohort for server filtering, inspects it, and preserves the population book', () => {
    const onGroupSelect = vi.fn(), onInspect = vi.fn();
    ...
    expect(onInspect).toHaveBeenCalledWith({ kind: 'economics-group', group: providerA, groupBy: 'provider' });
  ...
  it('uses server sorting for the complete ledger and never reorders only the loaded page', () => {
    ...
    expect(tableBody('Recorded requests').querySelector('tr').textContent).toContain('09-01 12:00:00');
    expect(container.textContent).toContain('Sorting and status filters apply to the complete contributing ledger.');
  });

  it('requests another server page and opens the exact record in the inspection dock', () => {
    const onPageChange = vi.fn(), onInspect = vi.fn();
  ...
  it('exposes pinning and switches grouping through controlled props', () => {
    ...
    click(container.querySelector('input[value="model"]'));
  ...
  it('does not turn an unassigned identity into an unfiltered request', () => {
    const onGroupSelect = vi.fn(), onInspect = vi.fn();
    mount({ data: { ...data, groups: [{ ...providerA, connectionId: null }] }, groupBy: 'account', onGroupSelect, onInspect });
  ...
  it('renders load, error and empty states without stale economics', () => {
    mount({ loading: true });
    expect(container.querySelector('[role="status"]').textContent).toContain('Loading recorded economics');
    ...
    mount({ data: { ...data, groups: [], items: [], summary: { ...quantities, records: 0, recordedCostUsd: null } } });
  ...
  it('retains unknown and inconsistent record data without inventing cache savings or a context join', () => {
    const record = { ...recordA, cacheWriteTokens: null, uncachedInputTokens: null, missingTokenDetail: 1 };
    act(() => root.render(<MantineProvider env="test"><EconomicsDetail selection={{ kind: 'economics-record', record }} /></MantineProvider>));
...
describe('Economics numerical and identity boundaries', () => {
  it('keeps shared model names distinct across providers', () => {
    ...
    expect(groupFilters({ ...providerA, connectionId: null }, 'account')).toBeNull();
  });
  it('uses persisted account names and a compact historical account fallback', () => {
    expect(groupName(providerA, 'account', [{ id: 'configured-1', name: 'Research account' }])).toBe('Research account');

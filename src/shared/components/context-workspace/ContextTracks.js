...
export function ContextTracks({ trend, scope, onScope }) {
  ...
  return <section className={styles.tracks} aria-label="Session evidence tracks">
    ...
    <div className={styles.trackCanvas}><div className={styles.trackLabels}><span><i style={{ background: METRIC_COLORS.selected }} />Peak estimate<small>tokens / bucket</small></span><span><i style={{ background: METRIC_COLORS.input }} />Input / cache read<small>provider token sums</small></span><span><i style={{ background: '#8291ae' }} />Body reduction<small>signed byte sum</small></span></div><AnalyticalChart option={option} height={260} label="Context estimates, provider input and cache reads, and signed body reduction across all filtered session attempts. Empty intervals are omitted. Select a bucket to focus the shared time range." onEvents={{ click: (event) => focus(points[event.dataIndex]) }} /></div>

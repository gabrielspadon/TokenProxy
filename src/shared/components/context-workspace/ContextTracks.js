...
export function ContextTracks({ trend, scope, onScope }) {
  ...
  const option = useMemo(() => trendOption(trend, METRIC_COLORS, scope, chartThemeColors(colorScheme)), [trend, scope, colorScheme]);
  ...
  return <section className={styles.tracks} aria-label="Session evidence tracks">
    ...
    <div className={styles.trackCanvas}><div className={styles.trackLabels}><span><i style={{ background: METRIC_COLORS.input }} />Peak estimate<small>tokens / bucket</small></span><span><i style={{ background: METRIC_COLORS.input }} />Input / cache read<small>provider tokens</small></span><span><i style={{ background: METRIC_COLORS.output }} />Body reduction<small>signed byte sum</small></span></div><AnalyticalChart option={option} height={150} label="Context estimates, provider input and cache reads, and signed body reduction across all filtered session attempts. Empty intervals are omitted. Select a bucket to focus the shared time range." onEvents={{ click: (event) => focus(points[event.dataIndex]) }} /></div>

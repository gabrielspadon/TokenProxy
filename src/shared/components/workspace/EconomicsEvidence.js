...
export function EconomicsEvidence({row,onDrilldown,onContext}) {
  ...
  return <div className={styles.evidence}>
    <div className={styles.models}><strong>Requested · {row.requestedModel || 'Unavailable'}</strong><span>Served · {row.provider || 'Unknown'} / {row.model || 'Unknown'}</span></div>
    ...
    <Tabs defaultValue="cost" keepMounted={false}>
      <Tabs.List aria-label="Economics evidence"><Tabs.Tab value="cost">Cost basis</Tabs.Tab><Tabs.Tab value="rates">Rate calculation</Tabs.Tab><Tabs.Tab value="identity">Exact links</Tabs.Tab></Tabs.List>
      ...
      <Tabs.Panel value="identity" pt="sm"><section className={styles.section}><h3>Exact identity and model evidence</h3><Facts items={[["Ledger record",row.id],["Physical request",row.requestId],["Logical request",row.logicalRequestId],["Explicit session",row.contextSessionId],["Requested model",row.requestedModel],["Served provider / model",`${row.provider || 'Unknown'} / ${row.model || 'Unknown'}`],["Client identity source",row.clientIdentitySource],["Client reference",row.clientRef],["Client session reference",row.clientSessionRef],["Task reference",row.taskRef],["Client project reference",row.projectRef],["Application project",row.projectId]]}/><p>Client references are installation-keyed hashes scoped to the authenticated key and client. They do not establish a person, cross-client project, or task outcome. Request-side links may become unavailable after retention. No timestamp-based attribution is used.</p></section></Tabs.Panel>
    </Tabs>
  </div>;
}

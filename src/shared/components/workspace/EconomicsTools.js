...
export default function EconomicsTools({ children, filters, filterCount = 0 }) {
  ...
  const tool = ['filters', 'pricing', 'budgets', 'projects'].includes(selected) ? selected : 'analysis';
  const [visited, setVisited] = useState(() => [tool]);
  if (!visited.includes(tool)) setVisited(previous => [...previous, tool]);
  const setTool = (value, projectId) => {
  ...
  return <Tabs className={styles.tasks} value={tool} onChange={setTool}>
    <Tabs.List aria-label="Economics tasks"><Tabs.Tab value="analysis">Analysis</Tabs.Tab><Tabs.Tab value="filters">Identity filters{filterCount ? ` (${filterCount})` : ''}</Tabs.Tab><Tabs.Tab value="pricing">Pricing</Tabs.Tab><Tabs.Tab value="budgets">Budget reservations</Tabs.Tab><Tabs.Tab value="projects">Projects</Tabs.Tab></Tabs.List>
    ...
    <Tabs.Panel className={styles.taskPanel} value="projects">{visited.includes('projects') && <ProjectBudgets onAnalyze={projectId => setTool('analysis', projectId)} />}</Tabs.Panel>
  </Tabs>;
}

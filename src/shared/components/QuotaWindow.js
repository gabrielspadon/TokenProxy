...
export function QuotaWindow({ provider, name, window: w, horizonMs, now }) {
  ...
  const level = !known ? undefined : w.remaining === 0 ? "empty" : w.remaining / w.limit < 0.15 ? "low" : undefined;
  const reset = isEpoch(w.resetAt) ? null : w.resetAt;
  return (
    <div className="row window">
      <div className="who">
        <span className="name" data-i18n-skip>{name || provider}</span>
        <span className="sub"><span data-i18n-skip>{w.scope}</span></span>
      </div>
      <div>
        ...
        <div className="band-meta">
          {known ? <span data-i18n-skip>{fmtNum(w.remaining)} / {fmtNum(w.limit)}</span> : <span>Remaining is not known</span>}
          <span>{CONF[w.confidence] || w.confidence}</span>
        </div>
      </div>
      <div>
        <Ruler horizonMs={horizonMs} now={now} marks={reset ? [{ at: reset }] : []} />
        <div className="band-meta">
          {reset ? <span>Resets <span data-i18n-skip>{fmtRelative(reset, now)}</span></span> : <span>Reset time is not known</span>}
          {isEpoch(w.observedAt) ? <span>Never observed</span> : <span>Seen <span data-i18n-skip>{fmtRelative(w.observedAt, now)}</span></span>}
        </div>
      </div>
    </div>
  );

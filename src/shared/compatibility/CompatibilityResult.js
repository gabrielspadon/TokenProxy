...
export function CompatibilityResult({ packet, onExport, onCancel }) {
  ...
  return <section className={styles.result} aria-label="Selected compatibility run">
    ...
    {!result ? <p className={styles.empty}>{['queued', 'running'].includes(run.status) ? 'Waiting for this exact local run to return. You can cancel it.' : 'No conversion result was returned. The terminal receipt is retained.'}</p> : <>
      ...
      <div className={styles.metrics}><span>Input <b>{result.quantities.inputBytes.toLocaleString()} B</b></span><span>Output <b>{result.quantities.outputBytes.toLocaleString()} B</b></span><span>Translator <b>{result.quantities.translatorDurationMs.toFixed(2)} ms</b></span>{result.operation === 'stream' && <span>Events <b>{result.quantities.inputEvents} → {result.quantities.outputEvents}</b></span>}</div>

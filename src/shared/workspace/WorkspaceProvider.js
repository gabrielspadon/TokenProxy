...
export function analyticsUrl(scope, view = 'activity', extra = {}) {
  ...
  for (const key of ['start','end','provider','model','connectionId']) if (scope[key]) query.set(key, scope[key]);
...
export function WorkspaceProvider({ children }) {
  ...
  const observeSnapshot = useCallback((next) => setSnapshot((previous) => previous?.capturedAt === next.capturedAt && previous?.isolated === next.isolated ? previous : next), []);
  ...
  const refresh = () => { health.refresh(); quota.refresh(); activity.refresh(); };
  const value = { scope, setScope, snapshot, accounts, health, quota, models, activity, selectedAccountId, setSelectedAccountId, comparisonIds, setComparisonIds, refresh, observeSnapshot };

...
export function Investigations() {
  ...
  async function reloadVersion() {
    ...
    try {
      ...
      const response=await fetch(`/api/admin/investigations/${target.id}`);const body=await response.json();if(!response.ok)throw new Error(body.error);
      if (deleting) setDeleting(body);
      else { workspace.setSavedEntry(body);setName(body.name);setKind(body.kind); }
      entries.refresh();setNotice('Latest saved version loaded. Current filters and selected evidence are unchanged.');
    }
    catch(error){setFailure(error);} finally{setBusy(false);}
  }
  return <>
    ...
    <Modal title="Saved investigations" opened={open} onClose={()=>{if(!busy){setOpen(false);setDeleting(null);}}} size="lg" closeButtonProps={{'aria-label':'Close saved investigations'}}>
      <Stack gap="md">
        <Text size="sm" c="var(--slate)">Shared by this installation’s authenticated operators. Saved ranges use fixed UTC boundaries. No request content or credentials are stored.</Text>
        ...
        {notice && <Text role="status" size="sm" c="var(--slate)" className={styles.notice}>{notice}</Text>}
        {deleting ? <Alert title={`Delete “${deleting.name}”?`} color="orange">The stored definition will be removed; evidence records are untouched.<Group mt="sm"><Button color="red" loading={busy} onClick={remove}>Delete saved entry</Button><Button variant="default" disabled={busy} onClick={()=>{setDeleting(null);setFailure(null);}}>Cancel deletion</Button></Group></Alert> : <form onSubmit={(event)=>{event.preventDefault();save(false);}}>
          ...
          <Text size="sm" c="var(--slate)" mt="xs">{kind==='filter-set'?'Filters only. Restoring this set retains your selected evidence.':kind==='bookmark'?'The exact selected identity, its fixed scope and comparison accounts.':'Current lens, filters, selected identity, account comparison and lens controls.'}</Text>
        ...
        <section aria-label="Stored investigations" className={styles.entries}>
          ...
          {!entries.loading && entries.data?.items?.length===0 && <Text c="var(--slate)">No saved definitions yet.</Text>}
          {entries.data?.items?.map((entry)=><div key={entry.id} className={styles.entry}><div><strong>{entry.name}</strong><Text size="sm" c="var(--slate)">{kindName[entry.kind]} · {entry.definition.lens} · version {entry.version}</Text></div><Group gap="xs"><Button size="compact-sm" variant="light" aria-label={`Restore ${entry.name}`} onClick={()=>restore(entry)}>Restore</Button><Button size="compact-sm" variant="subtle" color="red" aria-label={`Delete ${entry.name}`} onClick={()=>{setDeleting(entry);setFailure(null);}}>Delete</Button></Group></div>)}
        </section>
      </Stack>
    </Modal>
  </>;

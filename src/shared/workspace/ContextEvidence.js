...
function BoundaryTable({ caption, structures, rows }) {
  ...
  return <div className={styles.tableScroll}><Table className={styles.table} aria-label={caption}>
...
export function ContextAttemptComparison({ turn, baseline, onBaseline, onClear }) {
  ...
  return <section className={styles.evidence} aria-label="Attempt comparison">
    <div className={styles.intro}><h3>Compare exact attempts</h3><p>Choose a baseline explicitly, then inspect another attempt. The retained baseline stays fixed across page and scope changes.</p><Group gap={8}><Button variant="default" size="compact-sm" onClick={()=>onBaseline(turn)}>Use this attempt as baseline</Button>{old && <Button variant="subtle" size="compact-sm" onClick={onClear}>Clear baseline</Button>}</Group></div>
    {!old ? <p className={styles.empty}>No comparison baseline selected.</p> : <>
      ...
      <div className={styles.tableScroll}><Table className={styles.table} aria-label="Attempt measurement comparison"><Table.Caption>Selected minus baseline, each quantity retains its unit</Table.Caption><Table.Thead><Table.Tr><Table.Th>Measurement</Table.Th><Table.Th>Baseline</Table.Th><Table.Th>Selected</Table.Th><Table.Th>Change</Table.Th></Table.Tr></Table.Thead><Table.Tbody>

...
function PolicyForm({ rule, policy, effect, refresh }) {
  ...
  return <form className={styles.automationForm} aria-label="Account automation policy" onSubmit={event=>{event.preventDefault();run('save-policy');}}>
    <Group justify="space-between" gap="xs"><h4>Automatic drain</h4><Switch label="Allow automatic drain" checked={draft.enabled} disabled={busy||!rule.enabled} onChange={event=>set({enabled:event.currentTarget.checked})}/></Group>
    ...
    {!rule.enabled && <Text size="sm">Enable and save this rule before enabling its account action.</Text>}
    <div className={styles.automationFields}>
      <NumberInput label="Action cooldown (seconds)" value={draft.cooldownSeconds} min={60} max={86400} step={60} decimalScale={0} onChange={value=>set({cooldownSeconds:value})}/>
      <NumberInput label="Maximum actions per day" value={draft.dailyLimit} min={1} max={10} decimalScale={0} onChange={value=>set({dailyLimit:value})}/>
      <NumberInput label="Maximum alert age (seconds)" value={draft.maxEvidenceAgeSeconds} min={60} max={3600} step={60} decimalScale={0} onChange={value=>set({maxEvidenceAgeSeconds:value})}/>
    </div>
    ...
    {feedback && <Text role={feedback.ok?'status':'alert'} size="sm" c={feedback.ok?'dimmed':'orange'}>{feedback.text}</Text>}
    {preview && <section aria-label="Automation simulation"><Text size="sm" fw={600}>{preview.history.filter(row=>row.simulation.outcome==='drain').length} simulated drains · {preview.history.length} retained alerts</Text>
      <ul className={styles.note}>{preview.assumptions.map(text=><li key={text}>{text}</li>)}</ul>
      {preview.coverage.hasMore && <Text size="sm">Only the latest {preview.coverage.limit} alerts are included.</Text>}
      ...
      <Text size="sm">Simulation only. No historical alert is queued or executed.</Text>
    </section>}
  </form>;
}
function ActionHistory({ ruleId, refreshPolicy }) {
  ...
  return <section className={styles.automationHistory} aria-label="Account action history">
    ...
    <ol className={styles.audit}>{resource.data?.items.map(action=><li key={action.id}>
      ...
      {action.reasonCode && <Text size="sm">{action.reasonCode.replaceAll('_',' ')}</Text>}
    ...
    {feedback&&<Text size="sm" role={feedback.ok?'status':'alert'}>{feedback.text}</Text>}
...
export function NotificationAutomation({ rule }) {
  ...
  return <section className={styles.automation} aria-label="Bounded account automation">
    ...
    </>:<Text size="sm">This condition has no supported account action.</Text>)}
  </section>;
}

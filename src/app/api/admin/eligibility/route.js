...
export async function GET(request) {
  ...
  try {
    const [connections, windowsByConnection, drains, qualifications, settings, disabledModels] = await Promise.all([
      getProviderConnections(), getAllWindows(), readAllDrainDocs(), readAllQualifications(), getSettings(), getDisabledModels(),
    ]);
    return adminJson(projectEligibility({ connections, windowsByConnection, drains, qualifications, settings, disabledModels, provider, model }));
  } catch {

...
import { listSessionPins, getSessionPinAction, applySessionPin, PinControlError } from '@/lib/db/repos/sessionPinsRepo.js';
...
async function bodyOf(request, fields) {
  ...
  if (length !== null && (!/^\d+$/.test(length) || Number(length) > 32768)) throw new PinControlError('body_too_large', 413);
  ...
  if (reader) {
    try {
      while (true) {
        ...
        if (size > 32768) { void reader.cancel().catch(() => {}); throw new PinControlError('body_too_large', 413); }
        chunks.push(value);
      }
    } finally { reader.releaseLock(); }
  }
  let body; try { body = JSON.parse(Buffer.concat(chunks).toString('utf8')); } catch { throw new PinControlError('invalid_json'); }
  if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(k => !fields.includes(k))) throw new PinControlError('invalid_fields');
  return body;
}
async function handle(request, context) {
  ...
  try {
    ...
    if (error instanceof PinControlError) return adminError(error.status, error.code, 'Pin control input or state was refused.');
    if (error?.name === 'SimulationError') return adminError(error.status || 422, 'pin_topology_unavailable', 'This pin topology cannot be previewed.');

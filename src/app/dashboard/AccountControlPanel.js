...
function QuotaMeter({ window, mode, now, onInspect, threshold, onThresholdChange, disabled }) {
  ...
  const retained = accountWindowStale(window, now);
  ...
  return <div className={styles.window} data-retained={retained || !known || undefined}>
    ...
    <div className={styles.windowMeta}><span title={reset.absolute || undefined}>{reset.label}{window.threshold > 0 && !window.unlimited ? <span className={styles.pauseThreshold}> · Pause ≤ {number(window.threshold)}% left</span> : null}</span><span aria-label={`${observed.label}${retained && observed.absolute ? ', stale' : ''}`} title={observed.absolute || undefined}>{observed.label}{retained && observed.absolute ? ' · stale' : ''}</span></div>
    <div className={styles.thresholdControl}><NumberInput label="Auto-pause ≤" aria-label={`Auto-pause threshold for ${window.key}`} suffix="%" min={0} max={100} value={threshold} placeholder="Unknown" onChange={onThresholdChange} disabled={disabled} hideControls /><span>{window.unlimited ? 'Unlimited, not applied' : 'Remaining · 0 is off'}</span></div>
  </div>;
}
...
export function AccountControlPanel({ rows = EMPTY, scope = {}, selectedAccountId, onSelect, onChanged, anchor, comparisonIds = EMPTY, onComparisonChange, onCompare }) {
  ...
  function discard(id, form) {
    ...
    if (input && !input.disabled) input.focus(); else form?.focus();
  }
  ...
  return <section tabIndex={-1} className={styles.panel} aria-label="Account control panel" data-density={preferences?.density || 'comfortable'}>
    <div className={styles.controls}>
      <div className={styles.heading}><div><h2>Your accounts</h2><Text size="sm" c="dimmed">{matching.length} of {scoped.length} in scope</Text></div><TextInput className={styles.search} aria-label="Search accounts" placeholder="Search accounts or providers" type="search" value={query} onChange={event => setQuery(event.currentTarget.value)} /><Group gap="xs" className={styles.headingActions}><Button ref={customizeOrigin} size="compact-sm" variant="default" aria-expanded={customize} onClick={() => setCustomize(previous => !previous)}>Customize</Button><Button size="compact-sm" variant="default" onClick={refresh}>Refresh</Button></Group></div>
      <div className={styles.statusStrip} role="group" aria-label="Account status filters">{[STATES.slice(0, 3), STATES.slice(3, 6), STATES.slice(6)].map(group => <div key={group[0].value} className={styles.statusGroup}>{group.map(item => <Button key={item.value} variant={status === item.value ? 'light' : 'subtle'} color={item.value === 'Quota pause' || item.value === 'Needs attention' ? 'orange' : 'gray'} size="compact-sm" aria-pressed={status === item.value} onClick={() => setStatus(item.value)}>{item.label} {scoped.filter(account => matchesState(account, item.value)).length}</Button>)}</div>)}</div>
      <div className={styles.toolbar}><NativeSelect aria-label="Sort accounts" value={sort} onChange={event => { setSort(event.currentTarget.value); setSortAt(now); }} data={[{ value: 'name', label: 'Account name' }, { value: 'reset', label: 'Next observed reset' }, { value: 'headroom', label: 'Least recorded headroom' }]} /><SegmentedControl aria-label="Capacity measure" value={mode} onChange={setMode} data={[{ value: 'remaining', label: 'Remaining' }, { value: 'used', label: 'Used' }]} /><SegmentedControl aria-label="Account view" value={view} onChange={setView} data={[{ value: 'cards', label: 'Cards' }, { value: 'rows', label: 'Rows' }]} />{onComparisonChange ? <Group gap="xs" className={styles.comparisonActions}><Button variant="light" disabled={selected.length < 2 || selected.length > 4 || !onCompare} onClick={() => onCompare?.()}>Compare selected ({selected.length})</Button>{selected.length ? <Button variant="subtle" onClick={() => onComparisonChange([])}>Clear selection</Button> : null}</Group> : null}</div>
    ...
    {scope.start || scope.end ? <Text size="sm" c="dimmed" mb="sm">Attempt period {scope.start || 'earliest retained'} to {scope.end || 'latest retained'}. Capacity uses stored observations.</Text> : null}
    ...
    <div className={view === 'rows' ? styles.rows : styles.cards}>{matching.map(account => {
      ...
      return <article key={id} aria-label={name} className={styles.card} data-selected={selectedAccountId === id || undefined} data-comparing={selected.includes(id) || undefined} data-account-id={id}>
        ...
        <div className={styles.windows}>{visibleWindows.map(window => <QuotaMeter key={window.key} window={window} now={now} mode={mode} onInspect={() => onSelect?.(id, window.key)} threshold={draft ? draft.thresholds[window.key] ?? 0 : baseline ? baseline.quotaPauseThresholds[window.key] ?? 0 : ''} onThresholdChange={value => edit(account, baseline, 'threshold', value, window.key)} disabled={!canEdit} />)}{!windows.length ? <div className={styles.emptyWindow}>No quota window recorded. Usage and reset are unknown.</div> : !visibleWindows.length ? <Text size="sm" c="dimmed">All windows hidden in Customize.</Text> : null}</div>
        <details className={styles.evidenceDetails}><summary>Observation &amp; policy details</summary><div><p>Configuration and stored health do not establish model access. Other routing gates can still apply.</p><p>{accountWindowTime(evidence.observedAt, now).absolute ? `Health recorded ${accountWindowTime(evidence.observedAt, now).absolute}` : 'Health observation time unknown.'}</p>{visibleWindows.map(window => <div className={styles.windowEvidence} key={window.key}><strong>{window.key}</strong><span>{window.unlimited ? 'Auto-pause not applicable' : window.threshold > 0 ? `Pause ≤ ${number(window.threshold)}% left` : 'Auto-pause off'}</span><span>Observed {accountWindowTime(window.observedAt, now).absolute || 'at an unknown time'}</span><span>Reset {accountWindowTime(window.resetAt, now, true).absolute || 'time unknown'}</span></div>)}</div></details>
        <footer className={styles.cardFooter}><NumberInput className={styles.priorityControl} label="Priority" aria-label="Fallback priority" min={1} allowDecimal={false} value={draft ? draft.priority : baseline?.priority ?? ''} placeholder={baseline ? 'Unset' : 'Unknown'} onChange={value => edit(account, baseline, 'priority', value)} disabled={!canEdit} hideControls /><Group gap="xs"><Button size="compact-sm" variant="default" loading={busy[id] === 'pause'} disabled={!!busy[id] || notice?.blocked || typeof account.isActive !== 'boolean'} onClick={() => pause(account)}>{account.isActive === false ? 'Resume' : 'Pause'}</Button><Button size="compact-sm" variant="subtle" onClick={() => onSelect?.(id)}>Details</Button></Group>{Number.isFinite(account.activity?.records) ? <span>{number(account.activity.records)} attempts in selected period</span> : null}</footer>

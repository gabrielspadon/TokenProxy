...
function QuotaMeter({ window, mode, now, onInspect, threshold, onThresholdChange, disabled }) {
  ...
  return <div className={styles.window} data-retained={retained || !known || undefined}>
    ...
    <div className={styles.thresholdControl}>{Number.isFinite(threshold) && !window.unlimited ? <input className={styles.reserveSlider} type="range" min="0" max="100" step="1" value={threshold} aria-label={`Adjust auto-pause for ${window.key}`} aria-valuetext={`${number(threshold)} percent remaining; zero turns auto-pause off`} onChange={event => onThresholdChange(Number(event.currentTarget.value))} disabled={disabled} /> : null}<NumberInput label="Pause at" aria-label={`Auto-pause threshold for ${window.key}`} suffix="%" min={0} max={100} value={threshold} placeholder="Unknown" onChange={onThresholdChange} disabled={disabled} hideControls /><span>{window.unlimited ? 'Unlimited, not applied' : 'left · 0% is off'}</span></div>
  </div>;
}

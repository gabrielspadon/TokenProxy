...
function QuotaMeter({ window, mode, now, onInspect, threshold, onThresholdChange, disabled }) {
  ...
  return <div className={styles.window} data-retained={retained || !known || undefined}>
    ...
    <div className={styles.windowMeta}><span title={reset.absolute || undefined}>{reset.label}{window.threshold > 0 && !window.unlimited ? <span className={styles.pauseThreshold}> · Pause ≤ {number(window.threshold)}% left</span> : null}</span><span title={observed.absolute || undefined}>{observed.label}{retained && observed.absolute ? ' · stale' : ''}</span></div>

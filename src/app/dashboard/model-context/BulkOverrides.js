...
export function BulkOverrides({ overrides, disabled, onReadback }) {
  ...
  return <><Button variant="default" disabled={disabled} onClick={() => setOpen(true)}>Edit several overrides</Button><Modal opened={open} onClose={() => { if (!busy) setOpen(false); }} title="Edit several context-window overrides" size="lg"><Stack gap="md">
    ...
    <Textarea label="Overrides to set" description="One exact key = token limit per line." minRows={4} value={setText} disabled={busy || !!review} onChange={event => setSetText(event.currentTarget.value)} placeholder={'openai/example-model = 128000\nexample-* = 64000'} />
    <Textarea label="Override keys to remove" description="One exact saved key per line. Removing exposes the next matching rule or default." minRows={3} value={removeText} disabled={busy || !!review} onChange={event => setRemoveText(event.currentTarget.value)} />
    {review ? <><h3>Review exact key changes</h3><div className="model-context-bulk-review" role="region" aria-label="Reviewed context override changes" tabIndex={0}><dl>{review.set.map(item => <div key={item.key}><dt><code>{item.key}</code></dt><dd>{show(review.expectedOverrides[item.key])} → {show(item.contextWindow)}</dd></div>)}{review.deleteKeys.map(key => <div key={key}><dt><code>{key}</code></dt><dd>{show(review.expectedOverrides[key])} → Remove saved key</dd></div>)}</dl></div><Text>Only these keys are checked and changed together. Concurrent changes to a reviewed key are refused; concurrent sibling keys remain. Restore these previous values to reverse future behavior. The readback is not a retained change-history service.</Text><Group><Button loading={busy} disabled={uncertain} onClick={save}>Save reviewed overrides</Button><Button variant="default" disabled={busy} onClick={() => { setReview(null); setUncertain(false); }}>Return to retained draft</Button></Group></> : <Button onClick={prepare}>Review exact keys</Button>}
  </Stack></Modal></>;
}

...
function ExactKey({ children }) {
  return <code className="model-context-key" dir="ltr" data-i18n-skip><bdi dir="ltr">{children}</bdi></code>;
}

function TokenValue({ value }) {
  return Number.isFinite(value) && value > 0 ? <bdi className="model-context-number" dir="ltr" data-i18n-skip>{tokens(value)}</bdi> : <>Unknown</>;
}
...
export default function ModelContextPage() {
  ...
  const inspector = <div className="model-context-inspector" id="model-context-inspector">
    ...
    {!selectedExists ? <p>The selected record is no longer in the returned configuration.</p> : <>
      ...
      <div className="model-context-edit">
        ...
        <TextInput label="Context window in tokens" description="Positive whole tokens. Unknown limits stay blank." dir="ltr" attributes={{ input: { dir: 'ltr' } }} data-context-token-input data-i18n-skip inputMode="numeric" value={draft} onChange={event => setDraft(event.currentTarget.value)} error={value === null ? 'Enter a positive whole number of tokens.' : null} disabled={!canEdit} />
  ...
  return <div className="model-context-page">
    ...
    <div className="model-context-tools"><TextInput type="search" label="Find a model or saved key" placeholder="Provider, model identity or wildcard" value={query} onChange={event => { setQuery(event.currentTarget.value); setPage(0); }} /><NativeSelect label="Inventory" value={view} onChange={event => { setView(event.currentTarget.value); setPage(0); }} data={[{ value: 'models', label: 'Registered models' }, { value: 'overrides', label: 'Saved override keys' }]} /><span>{data ? <><bdi dir="ltr" data-i18n-skip>{rows.length.toLocaleString('en-US')}</bdi> {view === 'models' ? 'models' : 'saved keys'}</> : 'Inventory unknown'}</span></div>
    <div className="model-context-dock"><SelectionDock open={Boolean(selection)} title={model ? <bdi dir="auto" data-i18n-skip>{model.name || model.model}</bdi> : selection ? <bdi dir="ltr" data-i18n-skip>{selection.key}</bdi> : 'Context override'} subtitle={model ? <ExactKey>{identity(model)}</ExactKey> : 'Saved configuration key'} mark={model ? <ProviderMark provider={model.provider} /> : null} onClose={() => { setSelection(null); setNotice(null); }} detail={inspector} height="100%">
      <div className="model-context-inventory">
        <div className="model-context-table-scroll"><table><caption>{view === 'models' ? 'Registered and effective context windows, in tokens' : 'Persisted overrides in matching order, including rules without a catalog model'}</caption><thead><tr>{view === 'models' ? <><th>Configured identity</th><th>Catalog tokens</th><th>Effective tokens</th><th>Override source</th></> : <><th>Exact saved key</th><th>Tokens</th><th>Scope</th></>}</tr></thead><tbody>{visible.map(row => {
          if (view === 'overrides') return <tr key={row} data-selected={selection?.kind === 'override' && selection.key === row || undefined}><th><button type="button" aria-controls="model-context-inspector" aria-current={selection?.kind === 'override' && selection.key === row || undefined} onClick={() => select('override', row)}><ExactKey>{row}</ExactKey></button></th><td><TokenValue value={overrides[row]} /></td><td>{row.includes('*') ? 'Wildcard rule' : 'Exact key'}</td></tr>;
          const key = identity(row);
          const winner = resolveWindowOverride(overrides, row.provider, row.model);
          return <tr key={key} data-selected={selection?.kind === 'model' && selection.key === key || undefined}><th><button type="button" aria-controls="model-context-inspector" aria-current={selection?.kind === 'model' && selection.key === key || undefined} onClick={() => select('model', key, row)}><ProviderMark provider={row.provider} /><span><bdi dir="auto" data-i18n-skip>{row.name || row.model}</bdi><ExactKey>{key}</ExactKey><small><bdi dir="auto" data-i18n-skip>{row.providerName || row.provider}</bdi> · {Number.isFinite(row.providerConnections) ? <><bdi dir="ltr" data-i18n-skip>{row.providerConnections}</bdi> configured connections</> : 'Connections unknown'}</small></span></button></th><td><TokenValue value={row.staticContextWindow} /></td><td><TokenValue value={row.contextWindow} /></td><td>{winner ? <ExactKey>{winner.key}</ExactKey> : 'No context override'}</td></tr>;
        ...
        <nav className="model-context-pagination" aria-label="Context inventory pages"><Button variant="default" disabled={!currentPage} onClick={() => setPage(currentPage - 1)}>Previous</Button><span>Page <bdi dir="ltr" data-i18n-skip>{currentPage + 1}</bdi> of <bdi dir="ltr" data-i18n-skip>{pageCount}</bdi> · <bdi dir="ltr" data-i18n-skip>{PAGE_SIZE}</bdi> rows per page</span><Button variant="default" disabled={currentPage >= pageCount - 1} onClick={() => setPage(currentPage + 1)}>Next</Button></nav>
      </div>
    </SelectionDock></div>

...
export function AccountBoard({ rows, drains, anchor, now, advanced, density, onDensity, onChanged }) {
  ...
  // Refresh re-reads quota from every account on the board (a live provider
  // read per account, three at a time, honouring the probe gate), then reads
  // the retained evidence back. An isolated snapshot has no providers to ask,
  // so it only re-reads what is retained.
  const [reading, setReading] = useState(null);
  async function readQuota(id) {
    for (let attempt = 0; attempt < 5; attempt += 1) {
      const response = await call(`/api/usage/${encodeURIComponent(id)}`);

...
import { Accordion, Alert, Button, Checkbox, FileInput, Group, Modal, NativeSelect, NumberInput, Stack, Table, Text, Textarea, TextInput } from '@mantine/core';
...
export function CatalogTools() {
  ...
  return <Stack className={styles.tools} gap="lg">
    ...
    <Accordion multiple defaultValue={['import']} variant="contained">
      {ITEMS.map(([id, label]) => <Accordion.Item key={id} value={id}><Accordion.Control>{label}</Accordion.Control><Accordion.Panel>
        {id === 'import' && <Stack>

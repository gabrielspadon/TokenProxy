...
export default function ConnectionPage({ params }) {
  ...
  const savedControl = kind => kind === 'priority' ? c?.priority ?? 1
    : kind === 'thresholds' ? c?.quotaPauseThresholds || {}
      : kind === 'endpoint' ? { baseUrl: psd.baseUrl || '', apiType: psd.apiType || '' }
        : kind === 'pool' ? Object.fromEntries(['proxyPoolId', 'connectionProxyMode', 'connectionProxyEnabled', 'connectionNoProxy', 'strictProxy'].map(key => [key, psd[key] ?? null])) : null;

  function openAction(kind, seed = {}) {
    setRefused(null);
    setGrantStep(null);
    setField(seed);
    setReviewBaseline(savedControl(kind));
    setAction(kind);
  }
  ...
  const close = () => { if (busy) return; setAction(null); setBusy(false); setRefused(null); setField({}); };

  async function run() {
    if (JSON.stringify(savedControl(action)) !== JSON.stringify(reviewBaseline)) {
      setRefused({ tone: 'warn', title: 'Saved account controls changed after this review opened.', next: 'Cancel this review and load current saved values. Your draft remains available.' });
      return;
    }
    setBusy(true);
    setRefused(null);
    ...
    if (r.ok && action === 'reauth') {
      ...
      if (!read.ok || read.body?.connection?.id !== id || read.body.connection.provider !== c.provider) { setBusy(false); setUncertain(true); setRefused({ tone: 'warn', title: 'Credential replacement was accepted, but the selected account could not be read back.', next: 'Close and refresh before another change.' }); return; }
    }
    setBusy(false);
    if (r.ok) {
      close();
  ...
  const renderFields = (action, field, setField) => <>
        {action === "priority" ? (
          ...
          <div className="connections-form">
            {thresholdWindows.map((scope) => (
              <label className="field" key={scope}>
                <span>{scope}</span>
                <input className="input" type="number" min="0" max="100" step="any" placeholder="No pause"
  ...
  const taskForm = (kind, title, seed) => <ConnectionTaskForm key={`${id}-${kind}`} title={title} seed={seed} disabled={busy || uncertain || Boolean(action)} onReview={draft => openAction(kind, draft)}>
    {(draft, setDraft) => renderFields(kind, draft, setDraft)}
  </ConnectionTaskForm>;
  ...
  return (
    <div className="screen">
      ...
      {uncertain ? <Notice tone="warn" title="Refresh this account before another change." next="The last accepted credential write could not be read back. Do not repeat it." /> : null}
      {conn.loading ? <div className="skeleton" aria-hidden="true" /> : null}

      {c ? (
        <>
          <nav className="verb-row" aria-label="Account tasks">{[['policy', 'Policy'], ['models', 'Models'], ['auth', 'Auth'], ['diagnostics', 'Diagnostics']].map(([value, label]) => <button type="button" className={task === value ? 'button' : 'button quiet'} key={value} aria-pressed={task === value} disabled={busy || uncertain || Boolean(action)} onClick={() => setTask(value)}>{label}</button>)}</nav>
          <div hidden={task !== 'policy'}>
          <section>
            <dl className="facts">
          ...
          <AccountOptions key={c.id} connection={c} onSaved={() => { conn.refresh(); qual.refresh(); }} />
          {taskForm('priority', 'Routing priority', { priority: String(c.priority ?? 1) })}
          {taskForm('thresholds', 'Quota pause thresholds', { ...(c.quotaPauseThresholds || {}) })}
          {taskForm('pool', 'Account network path', { poolId: psd.proxyPoolId || '__clear__' })}
          {taskForm('endpoint', 'Endpoint override', { baseUrl: psd.baseUrl || '', apiType: psd.apiType || '' })}
          <p className="caption"><Link href="/dashboard/connections#provider-policy" prefetch={false}>Provider policy</Link> controls the shared provider concurrency ceiling and timeout.</p>
          </div>
          {task === 'models' ? <>
            <section><h2>Services</h2><p className="caption">Registered adapter services. These do not establish this account&apos;s upstream entitlement.</p><p>{kinds.map(kind => KIND_WORD[kind] || kind).join(', ')}</p></section>
            <AccountModelAccess key={c.id} connection={c} />
          </> : null}
          {task === 'auth' ? <>
            {taskForm('reauth', 'Replace the credential', { force: false, secret: '', machineId: '' })}
          </> : null}
          {task === 'diagnostics' ? <>
          ...
          <section>
            ...
            {taskForm('recheck', 'Recheck connection', { force: false })}
          </section>
      ...
      <Confirm open={!!action} busy={busy || uncertain} refusal={refused}
        ...
        <p>{c?.name || c?.id}</p>
        {['priority', 'thresholds', 'endpoint'].includes(action) ? <dl className="facts">{Object.entries(field).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{String(value || 'Provider default')}</dd></div>)}</dl> : null}
        {action === 'reauth' ? <p>Credential values stay private. {field.force ? 'Account rebinding is enabled.' : 'The provider identity must match this account.'}</p> : null}
...
function ConnectionTaskForm({ title, seed, disabled, onReview, children }) {
  const { values, set, reset, forget, conflicts } = useSavedDraft(seed);
  useEffect(() => {
    const clear = () => { if (document.hidden) forget(['secret', 'document', 'proxyUrl']); };
    document.addEventListener('visibilitychange', clear);
    return () => document.removeEventListener('visibilitychange', clear);
  }, [forget]);
  const update = change => {
    const next = typeof change === 'function' ? change(values) : change;
    for (const [key, value] of Object.entries(next)) if (value !== values[key]) set(key, value);
  };
  return <section className="panel account-task-form"><h2>{title}</h2><form onSubmit={event => { event.preventDefault(); if (disabled || conflicts.length) return; onReview(values); forget(['secret', 'document', 'proxyUrl']); }}><fieldset disabled={disabled}>
    {children(values, update)}
    {conflicts.length ? <Notice tone="warn" title="Saved account fields changed while you were editing." next={`Your draft is retained. Load saved values before reviewing. Changed fields: ${conflicts.join(', ')}.`} /> : null}

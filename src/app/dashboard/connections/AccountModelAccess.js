...
export function AccountModelAccess({ connection }) {
  ...
  // The question stands where it was asked: under the exclude control, or under
  // the row of the model being allowed again, never in a layer over the list.
  const review = pending ? <InlineConfirm
    title={pending.kind === 'disable' ? 'Exclude model from this account' : 'Remove account model exclusion'}
    verb="Apply model policy"
  ...
  return <section className="account-model-access" aria-labelledby="account-model-access-title">
    ...
    <section aria-label="Edit model access"><h3>Edit model access</h3>
      <div className="connections-form">
        ...
        {pending?.kind === 'disable' ? review : null}
        {ids.map(id => <Fragment key={id}>
          <div className="verb-row"><bdi>{id}</bdi><button type="button" className="button quiet" disabled={busy || uncertain || !!policy.error} onClick={() => { setError(null); setPending({ kind: 'enable', model: id }); }}>Allow {id}</button></div>
          {pending?.kind === 'enable' && pending.model === id ? review : null}
        </Fragment>)}
      </div>
    </section>

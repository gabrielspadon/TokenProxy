...
export function TokenSavings({ settings, stageMap, recent, period, onPeriod, onToggle, onNavigate, renderThresholds, renderConfiguration, loading, unavailable, onRefresh, busy }) {
  ...
  return <section className="savings-overview" aria-label="Token savings control panel">
    <div className="savings-status-strip">
      ...
      <div><span>Plan overrides</span><strong>{settings ? fmtNum(Object.values(settings.comboStrategies || {}).filter(value => value && Object.hasOwn(value, 'tokenSaver')).length) : 'Unknown'}</strong><button type="button" className="link-button" onClick={() => onNavigate('Plan overrides')}>Review exceptions</button></div>
    ...
    <p className="shaping-caption">Stage bytes are historical measurements, not billed token or cost savings. Controls can share a stage; measurements are not added together.</p>
    <div className="savings-categories">{CONTROL_GROUPS.map(group => {
      ...
      return <section key={group} className="savings-category" aria-label={`${group} controls`}>
        <header><div><h2>{group}</h2><p>{DESCRIPTIONS[group]}</p></div><span className="savings-category-count">{settings ? `${enabled} / ${allMembers.length} on` : 'Unknown'}</span></header>
        <div className="savings-control-rows">{members.map(control => {
          const state = configuredState(settings, control);
          const unsupported = UNAVAILABLE_CONTROLS[control.key];
          const evidence = stageEvidence(stageMap, control.stage);
          const firstInStage = members.find(member => member.stage === control.stage) === control;
          return <article key={control.key} className="savings-control-card" data-state={state} data-savings-control={control.key}>
            <div className="savings-control-title"><h3>{control.name}</h3><p>{control.purpose}</p><span>{unsupported ? 'Runtime unavailable' : control.dependsOn && settings?.[control.dependsOn] !== true ? 'Parent control is not on globally' : control.override ? 'Plans can override' : 'Global setting'}</span></div>
            <div className="savings-control-effect"><p>{control.effect}</p><span className="savings-inline-measurement">{control.stage ? <>Stage bytes {evidence.measured ? <SignedBytes value={evidence.delta} /> : 'not measured'}{evidence.measured ? ` · ${fmtNum(evidence.measuredRecords)} / ${fmtNum(evidence.records)} records` : ''}</> : 'No dedicated byte measurement'}</span></div>
            <div className="savings-control-switch"><span className="shaping-state" data-state={state}>{state}</span><Switch aria-label={`Enable ${control.name}`} checked={state === 'On'} disabled={state === 'Unknown' || !!unsupported || busy} onChange={event => onToggle(control, event.currentTarget.checked)} /></div>
            {firstInStage ? renderThresholds?.(control.stage) : null}
            {renderConfiguration?.(control)}
            <ControlEvidence control={control} settings={settings} stageMap={stageMap} recent={recent} onInvestigate={() => onNavigate('Profiles and comparison')} />
          </article>;
        })}</div>
      </section>;
    })}</div>
    ...
    <section id="savings-byte-evidence" className="savings-byte-evidence" hidden={!showEvidence} aria-label="Measured request bytes by stage">
      ...
      <Button variant="subtle" onClick={() => onNavigate('Recorded evidence')}>Open detailed evidence</Button>
    </section>
  </section>;
}

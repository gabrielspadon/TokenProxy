...
const REWRITE = "Every stored signing value is cleared, because the gateway never hands one back. Enter each one again.";
const MASK = "•••";
...
function maskUrl(raw) {
  try {
    ...
    const path = u.pathname.split("/").map((s) => (s.length >= 20 ? MASK : s)).join("/");
...
function DestinationForm({ endpoint, onCancel, onSubmit }) {
  ...
  const toggle = (name) => setEvents((es) => (es.includes(name) ? es.filter((e) => e !== name) : [...es, name]));
  const submit = (e) => {
    e.preventDefault();
    onSubmit({ id: endpoint ? endpoint.id : undefined, url: url.trim(), events, secret, active: endpoint ? endpoint.active : true });
    setSecret("");
  };
  return (
    <form className="notifications-form" onSubmit={submit}>
      <label className="field">
        <span>Address</span>
        <input className="input" type="url" required autoComplete="off" value={url} onChange={(e) => setUrl(e.target.value)} />
      </label>
      ...
      <p className="caption">Write-only. It signs every message and is never shown again. Left empty, no signing value is stored.</p>
      <div className="actions">
        <button type="submit" className="button">Save destination</button>
        {endpoint ? <button type="button" className="button quiet" onClick={onCancel}>Cancel</button> : null}
      </div>
    </form>
  );
}
...
function RateForm({ rate, onSubmit }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    onSubmit({ threshold: Number(percent) / 100, windowSeconds: Number(seconds), minSamples: Number(samples) });
  };
  return (
    <form className="notifications-form" onSubmit={submit}>
      <label className="field">
        <span>Error rate that counts as a problem, in percent</span>
        <input className="input" type="number" min="1" max="100" step="1" required value={percent} onChange={(e) => setPercent(e.target.value)} />
      </label>
      <label className="field">
        <span>Window, in seconds</span>
        <input className="input" type="number" min="60" max="86400" step="60" required value={seconds} onChange={(e) => setSeconds(e.target.value)} />
      </label>
      <label className="field">
        <span>Fewest requests worth judging on</span>
        <input className="input" type="number" min="1" max="100000" step="1" required value={samples} onChange={(e) => setSamples(e.target.value)} />
      </label>
      <div className="actions"><button type="submit" className="button">Save rule</button></div>
    </form>
  );
}
...
export default function NotificationsPage() {
  ...
  const confirm = async () => {
    ...
    setBusy(false);
    if (!res.ok) { setDeny(refusal(res.status, res.body)); return; }
    setDone(ask.done ? ask.done(res.body) : null);
    setAsk(null);
    setEditing(null);
    notif.refresh();
  };
  const close = () => { setAsk(null); setDeny(null); };

  const saveDestination = (draft) => setAsk({
    ...
    run: put({
      endpoints: draft.id
        ...
        : [...endpoints.map(wire), { url: draft.url, events: draft.events, active: true, secret: draft.secret }],
    }),
  ...
  return (
    <>
      ...
      <section aria-labelledby="h-sending">
        <h2 id="h-sending">Sending</h2>
        {notif.loading ? <p className="skeleton">Reading</p> : null}
        {cfg ? (
          <>
            <dl className="facts">
              <dt>Sending</dt>
              <dd><span className="status" data-tone={cfg.enabled ? "ok" : undefined}>{cfg.enabled ? "On" : "Off"}</span></dd>
              <dt>Destinations</dt>
        ...
        <ul className="bullets">
          {Object.entries(EVENTS).map(([id, word]) => <li key={id}>{word}</li>)}
        </ul>
        <p className="caption">A message goes out when a condition changes, not while it lasts. An account is seeded silently the first time it is seen, so a restart does not replay an old incident. A destination that does not answer delays no request and fails none.</p>
      </section>

      <section aria-labelledby="h-where">
        <h2 id="h-where">Where they go</h2>
        {held ? <p className="caption">{REWRITE}</p> : null}
        {cfg && endpoints.length === 0 ? <p className="empty">No destination is configured. Add one below; until then nothing is sent anywhere.</p> : null}
        {endpoints.length ? (
          <div className="rows">
            <div className="row head notifications-dest"><span>Address</span><span>Events</span><span>Signing value</span><span>Actions</span></div>
            {endpoints.map((e) => (
              <div key={e.id} className="row notifications-dest">
                <span className="who">
                  <span className="name" data-i18n-skip>{maskUrl(e.url)}</span>
                  <span className="sub"><span className="status" data-tone={e.active ? "ok" : undefined}>{e.active ? "On" : "Off"}</span></span>
                </span>
                ...
                <span className="notifications-actions">
                  <button type="button" className="link-button" onClick={() => setAsk({
                    ...
                    irreversible: true,
                    run: () => call("/api/notifications/test", { method: "POST", body: { endpointId: e.id } }),
                    done: (b) => (b.ok
                      ? { tone: "ok", title: "Test sent", detail: `HTTP ${b.status}` }
                      : { tone: "bad", title: "The test did not arrive.", next: "Check the address, and that the receiver answers with a 2xx.", detail: b.error }),
                  })}>Send test</button>
                  <button type="button" className="link-button" onClick={() => setEditing(e.id)}>Change</button>
                  <button type="button" className="link-button" onClick={() => setAsk({
                    title: e.active ? "Turn this destination off" : "Turn this destination on",
                    verb: e.active ? "Turn off" : "Turn on",
                    requires: OPERATOR,
                    changes: changes(e.active
                    ...
                    run: put({ endpoints: endpoints.map((x) => (x.id === e.id ? { ...wire(x), active: !x.active } : wire(x))) }),
                    done: () => ({ tone: "ok", title: e.active ? "Destination turned off" : "Destination turned on" }),
                  })}>{e.active ? "Turn off" : "Turn on"}</button>
                  <button type="button" className="link-button" onClick={() => setAsk({
                    title: "Clear this destination",
                    verb: "Clear",
                    requires: OPERATOR,
                    changes: changes("The address is removed, and no further event is delivered there."),
        ...
        {editing ? (
          <DestinationForm key={editing} endpoint={endpoints.find((e) => e.id === editing)} onCancel={() => setEditing(null)} onSubmit={saveDestination} />
        ) : (
          <details>
      ...
      <section aria-labelledby="h-rate">
        <h2 id="h-rate">When the error rate counts as a problem</h2>
        <p className="caption">Below the fewest requests worth judging on, the rule does not evaluate at all, so a handful of failures is never an alarm.</p>
        {cfg ? (
          <RateForm
            key={`${cfg.errorRate.threshold}:${cfg.errorRate.windowSeconds}:${cfg.errorRate.minSamples}`}
            rate={cfg.errorRate}
            onSubmit={(errorRate) => setAsk({
              title: "Change the error-rate rule",
              verb: "Save rule",
              requires: OPERATOR,
              changes: "The rule fires at the new rate over the new window, once the new number of requests has been seen.",
      ...
      <section aria-labelledby="h-log">
        ...
        {notif.data && deliveries.length === 0 ? <p className="empty">Never sent. A delivery appears here once an event fires.</p> : null}
        {deliveries.length ? (
          <div className="rows">
            <div className="row head notifications-log"><span>Destination</span><span>Event</span><span>Outcome</span></div>
            {deliveries.map((d, i) => (
              <div key={`${d.at}-${i}`} className="row notifications-log">
                <span className="who">
                  <span className="name" data-i18n-skip>{named(d.endpointId)}</span>
                  <span className="sub" data-i18n-skip>{fmtRelative(d.at)}</span>
                </span>
                <span>{EVENTS[d.event] || <span data-i18n-skip>{d.event}</span>}</span>
                <span className="who">
                  <span className="status" data-tone={d.ok ? "ok" : "bad"}>{d.ok ? "Delivered" : "Not delivered"}</span>
                  <span className="sub" data-i18n-skip>{[d.status ? `HTTP ${d.status}` : null, d.attempts ? `${fmtNum(d.attempts)}x` : null, d.error].filter(Boolean).join(" ")}</span>
                </span>
              </div>
            ))}
          </div>
        ) : null}
        <p className="caption">The log holds the last fifty deliveries and lives inside the gateway process, so a restart empties it. A test send is not recorded here; its result appears beside the button that sent it.</p>
      </section>

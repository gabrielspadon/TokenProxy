...
* `GET /?limit=25&before=<pinId>` pages retained pins by their stable composite key, at most 50. Optional scope filters narrow the set before the cursor pages it: `provider` and `connectionId` and `model` match exactly, and `lastSeenFrom`/`lastSeenTo` bound the pin's recorded `lastSeenAt` (inclusive, canonical UTC ISO), never its expiry. Each parameter appears at most once; a repeated or unknown parameter is refused. Each row includes at most eight exact request joins, switch receipts and control actions, plus up to 201 same-provider target choices. Target lists at that bound may be incomplete; the simulator separately refuses captures over 200 accounts.

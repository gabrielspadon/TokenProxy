...
//
// Checks per route and width: page loads 200, console has no errors,
// no request leaves for a non-loopback host, axe reports zero serious or
// critical violations, no horizontal overflow, and English left-to-right text.
...
for (const width of widths) {
  ...
  for (const route of routes) {
    const tag = `${route.replace(/\W+/g, "_") || "root"}.en.${width}`;
    ...
    check(`${tag} English`, lang === "en" && dir === "ltr", `lang=${lang} dir=${dir}`);
  ...
  check(`en.${width} console`, consoleErrors.length === 0, consoleErrors[0] || "clean");
  check(`en.${width} third-party`, thirdParty.size === 0, [...thirdParty].join(",") || "none");
  await page.close();
}
await browser.close();

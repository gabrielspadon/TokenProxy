...
- `src/app/api/translator/translate/route.js:16` removes credential headers, URL credentials/query values and exact known credential values from local executor diagnostics. Steps 1 and 2 expose registered conversion paths and reject unsupported edges before account lookup. `src/app/dashboard/translation/page.js` exposes route evidence, invalidates downstream results when source or target changes, and distinguishes format detection, intermediate conversion, local executor construction and a real upstream send.
...
```bash
npx eslint src/shared/workspace/NotificationRules.js src/shared/workspace/OperationHistoryInspector.js src/shared/workspace/operationHistoryModel.js src/shared/compatibility/CompatibilityResult.js src/app/dashboard/compatibility/page.js src/app/dashboard/translation/page.js src/app/dashboard/system/page.js src/app/dashboard/tools/page.js src/app/api/translator/translate/route.js src/app/api/settings/database/route.js src/lib/db/analytics/operationEventsQueries.mjs src/lib/db/repos/compatibilityRepo.js src/lib/db/repos/notificationRulesRepo.js tests/unit/notification-rules-controls.test.js tests/unit/operations-retained-receipts.test.js tests/unit/database-import-partial.test.js tests/unit/translation-local-diagnostics.test.js tests/unit/compatibility-ui.test.js tests/e2e/operations-compatibility-persistence.spec.mjs tests/fixtures/operations-workspace-v1.mjs
```
...
The first all-route audit found Notifications contrast defects, now corrected in source. Rendered review also found ambiguous threshold units and an implementation-oriented unavailable-condition explanation; their source corrections passed 18 focused notification checks. Final rendering is pending. System and Remote still require scoped partial-read failure feedback; Tools requires removal of the large blank area while its inspector is closed.
...
System and Remote now keep source-specific refresh failures visible beside retained process, readiness, mesh and access-policy observations. Each retry reads only its failed source. A failed refresh does not silently turn the prior observation into current health. Compact System and Tools summaries preserve actual values and missing-data semantics.
...
On disposable fixture `S5cNBg`, the actual Notifications journey passed creation, stale-version refusal and recovery, readback, historical dry run, snooze and acknowledgement. The actual Compatibility journey passed fixture retention, pinned revision, local execution, exact result readback and terminal cancellation immutability. Its test now explicitly opens the fixture editor when populated, then returns from the responsive result drawer before refreshing the comparison. The six Translation/Tools journeys also passed, including actual local conversion and request-edit invalidation; Tools response variants remain intercepted presentation evidence. Across the domain, 15 distinct browser journeys passed in targeted runs. This is development-runtime evidence, not a single final-artifact run. No provider request, process start/stop control, real credential or live database was used.

...
The request inspector shows the three available structure boundaries, body and role partitions, overlapping tool/attachment subsets, installation-keyed fingerprints, and exact linked ledger amounts. A baseline attempt is explicitly chosen by exact request/session identity. Its labeled read stays fixed across page and scope changes, and the identity survives lens changes and saved restores. Differences retain their token or UTF-8-byte units; no cash saving or compaction is inferred.
...
Exact attempt comparisons use investigation definition version 2 with `context.baseline={id,sessionId}` and export mode `attempt-comparison`. The baseline identity is preserved across lenses and saved restores. Both exact tuples are re-read together for export, with missing members disclosed in `missingAttempts` and `comparisonComplete`; UI read times remain distinct from export freshness. Existing version 1 definitions and account comparisons retain their prior meaning.

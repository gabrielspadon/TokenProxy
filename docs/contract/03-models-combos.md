...
`GET` (full file, 127 lines): discovers unacknowledged newly-seen models via `reconcileSeenModels`/`getUnseenModels`/`countUnseenModels` (`@/models`), cached via `@/lib/newModelsCache`. Error path (120-125): `500 { error: "Failed to discover models", groups: [], total: 0, totalUnseen: 0, seeded: false }` — **error body still carries full success-shape with zero/empty defaults**, not a bare `{error}`. `seeded:false` on error; a genuine first-run bootstrap does set `seeded:true`. `reconcileSeenModels` (`src/lib/db/repos/seenModelsRepo.js:51,57-65`) treats an empty `seenModels` table plus a non-empty `observed` list as `isFirstScan`, inserts every observed model row with `acknowledged:1`, and sets `seeded=true`. The route (`src/app/api/models/new/route.js:60-63`) then short-circuits to `{ groups: [], total: 0, totalUnseen, seeded: true }` instead of surfacing the just-seeded rows as "new" — since every row it inserted this pass is already acknowledged, `totalUnseen` (`countUnseenModels()`) also comes back `0`, so the whole delta reads as empty on a genuine first run rather than a wall of new-model badges.
...
`POST /api/combos/[id]/test` (142 lines): body `{ prompt?, mode?: "fallback"|"all" }`. Uses `peekRotatedModels` — previews order WITHOUT mutating rotation state (peek vs mutating `getRotatedModels`). `mode:"all"` pings every member; default `"fallback"` does stop at first success. On the first `pingRes.ok`, `comboStatus` flips to `"success"`, every remaining member is pushed as a `skipped:true` step (`` `Skipped: Fallback satisfied by step #${i+1} (${modelStr})` ``, `[id]/test/route.js:96-107`), and the loop `break`s (`route.js:107`). If no member ever succeeds there is no such break, so `"fallback"` mode still pings every remaining member in order on a run of failures, same as `"all"` would, and ends with `comboStatus:"failed"` and zero `skipped` steps. `totalLatencyMs` = `steps.reduce((a,b)=>a+(b.latencyMs||0),0)` (~134) — **missing `latencyMs` treated as `0`, step not excluded**. `500 { error: "Failed to test combo execution" }`.

...
The response contains `asOf`, `mode:"passive"`, `requested:{provider,model,routePrefix}`, `basis:"persisted-local-gates"`, `upstreamVerified:false`, `limitations`, `capabilities` and `accounts`. `routePrefix` retains the submitted provider prefix for traceability; model-disable policy normalizes registered aliases exactly as routing does.
...
- `reasons` contains fixed safe labels and `{code,source,observedAt,until,effect}` evidence, with `effect` equal to `blocks-selection` or `context`. Gates cover account disablement, provider mismatch, draining, explicit model exclusions, active model/account locks, recorded quota thresholds and the real local quota ranker. The shared `isAccountModelDisabled` helper resolves aliases and applies each account's own disabled list, including an explicit empty list, before inheriting provider defaults. The provider-disable switch is enforced only for no-auth providers; authenticated-provider flags remain contextual notes. Legacy cooldown is display context, not an independent routing gate.

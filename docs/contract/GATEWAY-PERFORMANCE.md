...
The default custom `fixture-model` intentionally exercises unknown pricing/capability resolution. A known catalog comparison uses `--model=gpt-4o` and stays a separate workload. The multimodal scenario always uses the known vision-capable gpt-4o identifier; an unknown text-only model would correctly strip the image and invalidate that fixture's preservation claim. Large direct translation uses `--scenarios=translation-large`.
...
`--structure=off` creates a separate settings-only ablation. Every receipt retains the exact explicit fixture settings; compare it with a fresh `--structure=on` run of the same application build. This is diagnostic and must not become an automatic production feature disable. `--profile=true` enables a V8 CPU profile and native better-sqlite3 statement timing only during each gateway-only batch. It is a separate instrumented run, not an acceptance latency result. Statement counters classify operation/table without retaining SQL, parameter values or credentials. They time prepared run/get/all calls; native internal BEGIN/COMMIT/SAVEPOINT work appears in the CPU profile but is outside those counters. Profiler start/stop stacks must be excluded from application CPU attribution. Profile files remain private alongside the run database.
